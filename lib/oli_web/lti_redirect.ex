defmodule OliWeb.LtiRedirect do
  ...
  def redirect_from_launch(conn, %LaunchAttempt{} = attempt, opts \\ []) do
    ...
    with %Accounts.User{independent_learner: false} <- conn.assigns.current_user do
      case attempt.context_id do
        ...
        context_id ->
          ...
          section = resolved_launch_section(attempt)
  ...
  defp resolved_launch_section(%LaunchAttempt{resolved_section_id: nil}), do: nil

  defp resolved_launch_section(%LaunchAttempt{resolved_section_id: section_id}),
    do: Sections.get_section!(section_id)
