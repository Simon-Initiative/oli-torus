defmodule OliWeb.CollaborationLive.ShowPost do
  ...
  def render(assigns) do
    ...
    <div
      ...
      class={"accordion-item" <> if @post.status == :archived, do: " readonly", else: ""}
    >
              ...
              <div>
                {#if @post.user_id == @user.id}
                  ...
                  <span
                    ...
                    tabindex="0"
                    data-toggle="tooltip"
                    title={if @post.replies_count > 0, do: "Cannot be deleted because it has replies", else: "Delete"}
                  >
                    <button
                      class="btn btn-link"
                      type="button"
                      ...
                      disabled={if @post.replies_count > 0, do: true}
                  ...
                  {#if @post.replies_count > 0}
                    ...
                    >
                      <div class="d-flex align-items-center">
                        ...
                        <div><i class="fa fa-angle-down mr-1" /></div>
                      </div>
                    </button>
                  {/if}
                {/if}
          ...
          <div class="accordion-body">
            {#for {reply, reply_index} <- @post.replies}
              <div
                ...
                class={"card border-reply mb-3 p-2" <> if reply.status == :archived, do: " readonly", else: ""}
              >
                <div class="card-header d-flex justify-content-between align-items-center p-0">
                  <div class="d-flex align-items-center">
                    ...
                    <div>
                      {#if reply.user_id == @user.id}
                        ...
                        <span
                          ...
                          tabindex="0"
                          data-toggle="tooltip"
                          title={if has_replies?(@post.replies, reply.id),
                        ...
                        >
                          <button
                            ...
                            :on-click="display_delete_modal"
                            phx-value-id={reply.id}
                            phx-value-index={"#{@index}.#{reply_index}"}
                            disabled={has_replies?(@post.replies, reply.id)}
                          ><i class="fas fa-trash" /></button>
                        </span>
                      {/if}
                    </div>
