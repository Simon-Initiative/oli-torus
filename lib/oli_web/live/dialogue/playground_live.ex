defmodule OliWeb.Dialogue.PlaygroundLive do
  ...
  def mount(
    ...
    dialogue = build_page_prompt(session)
  ...
  def render(assigns) do
    ...
    <div style={window_style(@minimized)}>
      ...
      <div class="flex justify-between border-b-2 border-gray p-3">
        ...
        <div><i phx-click="minimize" class="cursor-pointer fa-regular fa-window-minimize mr-2"></i><i phx-click="restore" class="cursor-pointer fa-regular fa-window-restore"></i></div>
      </div>
  ...
  def render_input(assigns) do
    ...
    <.form :let={f} for={@changeset} phx-submit="update">
      <div class="relative">
        <%= textarea(f, :content,
          class: "resize-none ml-2 w-95 p-2 w-full rounded-md border-2 border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent",
          disabled: @streaming,
          required: true
        ) %>
    ...
    <div class={styles(:assistant)}>
      <%= if is_nil(@active_message) do %>
        <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <style>.spinner_b2T7{animation:spinner_xe7Q .8s linear infinite}.spinner_YRVV{animation-delay:-.65s}.spinner_c9oY{animation-delay:-.5s}@keyframes spinner_xe7Q{93.75%,100%{r:3px}46.875%{r:.2px}}</style><circle class="spinner_b2T7" cx="4" cy="12" r="3"/>
            <circle class="spinner_b2T7 spinner_YRVV" cx="12" cy="12" r="3"/><circle class="spinner_b2T7 spinner_c9oY" cx="20" cy="12" r="3"/>
        </svg>
      <% else %>
        <%= @active_message %>
      <% end %>
  ...
  def handle_info({:reply_finished}, socket) do
    ...
    case socket.assigns.function_call do
      ...
      fc ->
        ...
        dialogue = Dialogue.add_message(socket.assigns.dialogue, Message.new(:function, result, fc["name"]))
