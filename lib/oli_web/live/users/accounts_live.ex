defmodule OliWeb.Accounts.AccountsLive do
  ...
  def mount(_, %{"current_author_id" => author_id}, socket) do
    ...
    {:ok, model} = AccountsModel.new(users_model: users_model, authors_model: authors_model, author: author)
  ...
  defp get_patch_params(%AccountsModel{authors_model: authors_model, users_model: users_model, active_tab: active_tab}) do
  ...
  def handle_event("sort_authors", %{"sort_by" => sort_by}, socket) do
    ...
    authors_model = SortableTableModel.update_sort_params(socket.assigns.model.authors_model, String.to_existing_atom(sort_by))
    model = Map.put(socket.assigns.model, :authors_model, authors_model)

    {:noreply, push_patch(socket, to: Routes.live_path(socket, __MODULE__, get_patch_params(model)))}
  end

  def handle_event("sort_users", %{"sort_by" => sort_by}, socket) do
    users_model = SortableTableModel.update_sort_params(socket.assigns.model.users_model, String.to_existing_atom(sort_by))
  ...
  defp change_system_role(role_id, socket) do
    ...
    case Accounts.update_author(author, %{ system_role_id: role_id }) do
      {:ok, author} ->
        index = Enum.find_index(socket.assigns.model.authors_model.rows, fn a -> a.email == author.email end)
        rows = List.replace_at(socket.assigns.model.authors_model.rows, index, author)
        authors_model = Map.put(socket.assigns.model.authors_model, :rows, rows)
        |> Map.put(:selected, author)

        model = Map.put(socket.assigns.model, :authors_model, authors_model)
        assign(socket, model: model)

      {:error, _} -> put_flash(socket, :error, "Could not edit author")
    end
  ...
  def handle_params(params, _, socket) do
    ...
    authors_model = SortableTableModel.update_from_params(socket.assigns.model.authors_model, params)
    users_model = SortableTableModel.update_from_params(socket.assigns.model.users_model, params)

    model = AccountsModel.change_active_tab(socket.assigns.model, active_tab)
