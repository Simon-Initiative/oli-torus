defmodule OliWeb.Users.Common do
  ...
  def render_email_column(
          ...
          cond do
            Accounts.user_confirmation_pending?(row) ->
              ...
              <span data-toggle="tooltip" data-html="true" title={"Confirmation Pending sent to #{email}"}>
                <i class="fas fa-paper-plane text-secondary" />
              </span>
              """
            ...
            not is_nil(email_confirmed_at) ->
              ...
              <span
                data-toggle="tooltip"
                data-html="true"
                title={"Email Confirmed on #{Utils.render_precise_date(row, :email_confirmed_at, @context)}"}
              >
            ...
            not is_nil(invitation_accepted_at) ->
              ...
              <span
                data-toggle="tooltip"
                data-html="true"
                title={"Invitation Accepted on #{Utils.render_precise_date(row, :invitation_accepted_at, @context)}"}
              >
                <i class="fas fa-check text-success" />
              </span>
              """
    ...
    <div class="d-flex flex-row">
      {email} <div class="flex-grow-1" /> {checkmark}
    </div>
    <div>
      {#if !is_nil(locked_at)}
        <span class="badge badge-warning"><i class="fas fa-user-lock" /> Account Locked</span>
      {/if}
    </div>
    """
  end
