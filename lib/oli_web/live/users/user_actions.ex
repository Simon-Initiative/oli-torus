defmodule OliWeb.Users.Actions do
  ...
  attr(:include_account_actions, :boolean, default: true)
  attr(:act_as_enabled, :boolean, default: false)

  def render(assigns) do
    ~H"""
    <div>
      <%= if @include_account_actions do %>
      ...
      <.link
        :if={@act_as_enabled}
        navigate={~p"/admin/users/#{@user_id}/act_as"}
        class="btn btn-primary mt-2"
      >
        Act as user
      </.link>

      <div :if={@act_as_enabled} class="dropdown-divider"></div>

      <.button
        :if={@include_account_actions}
        variant={:danger}
        class="mt-6"
      ...
      <%= if !@include_account_actions and !@act_as_enabled do %>
