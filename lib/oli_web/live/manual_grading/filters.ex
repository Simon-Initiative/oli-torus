defmodule OliWeb.ManualGrading.Filters do
  ...
  def render(assigns) do
    ...
    <div style="display: inline;">
      <FilterButton
        selection={@selection}
        tooltip="Only show attempts from this same user"
        label="User"
        key={:user_id}
        active={is_active(assigns, :user_id)}
        clicked="filters_changed"
      />
      <FilterButton
        selection={@selection}
        tooltip="Only show attempts for this same activity"
        label="Activity"
        key={:activity_id}
        active={is_active(assigns, :activity_id)}
        clicked="filters_changed"
      />
      <FilterButton
        selection={@selection}
        tooltip="Only show attempts for this same page"
        label="Page"
        key={:page_id}
        active={is_active(assigns, :page_id)}
        clicked="filters_changed"
      />
      <FilterButton
        selection={@selection}
        tooltip="Only show attempts of this same purpose"
        label="Purpose"
        key={:graded}
        active={is_active(assigns, :graded)}
        clicked="filters_changed"
      />
    </div>
    """
