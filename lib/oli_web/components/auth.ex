defmodule OliWeb.Components.Auth do
  ...
  def registration_form(assigns) do
    ~H"""
    <div class={[
      "w-96 bg-Surface-surface-primary rounded-md sm:shadow-lg dark:text-white",
      @class
    ]}>
  ...
  @doc """
  Renders an account settings Google authentication button.

  ## Examples

  <.account_settings_google_auth_button href={~p"/users/auth/google/new"}>
    Connect Google authentication
  </.account_settings_google_auth_button>
  """
  attr :href, :string, required: true
  attr :user_return_to, :string, default: nil
  attr :class, :string, default: ""
  ...
  def account_settings_google_auth_button(assigns) do
    assigns =
      assign(assigns,
        navigate: maybe_user_return_to(assigns.href, assigns.user_return_to)
      )
    ...
    <.link
      ...
      class={[
        "border-[1.2px] border-Border-border-bold flex gap-2 items-center justify-center px-4 py-3 rounded-md w-full cursor-pointer hover:no-underline bg-Background-bg-primary",
        @class
      ]}
      {@rest}
    >
  ...
  @doc """
  Renders an account settings authorization link for connecting authentication providers.

  ## Examples

    <.account_settings_authorization_link provider={:google} href={~p"/users/auth/google/new"}>
      Connect Google authentication
    </.account_settings_authorization_link>
  """
  attr :provider, :atom, required: true
  attr :href, :string, required: true
  attr :user_return_to, :string, default: nil
  ...
  def account_settings_authorization_link(%{provider: :google} = assigns) do
    ~H"""
    <.account_settings_google_auth_button href={@href} user_return_to={@user_return_to} {@rest}>
      {render_slot(@inner_block)}
    </.account_settings_google_auth_button>
    """
  end

  def account_settings_authorization_link(assigns) do
    ~H"""
    <.link
      href={@href}
      aria-label={"Sign in with #{provider_title(@provider)}"}
      class={[
        "bg-white border border-gray-400 rounded-md p-2 text-body dark:text-body-dark text-center hover:no-underline",
        @class
      ]}
      {@rest}
    >
  ...
  @doc """
  Renders an account settings deauthorization link for removing authentication providers.

  ## Examples

    <.account_settings_deauthorization_link provider={:google} href={~p"/users/auth/google"}>
      Remove Google authentication
    </.account_settings_deauthorization_link>
  """
  attr :provider, :atom, required: true
  attr :href, :string, required: true
  attr :user_return_to, :string, default: nil
  ...
  def account_settings_deauthorization_link(%{provider: :google} = assigns) do
    ~H"""
    <.account_settings_google_auth_button
      href={@href}
  ...
  def account_settings_deauthorization_link(assigns) do
    ~H"""
    <.link
      href={@href}
      method="delete"
      aria-label={"Remove #{provider_title(@provider)} authentication"}
      class={[
        "bg-white border border-gray-400 rounded-md p-2 text-body dark:text-body-dark text-center hover:no-underline",
        @class
      ]}
