defmodule OliWeb.ObjectivesController do
  ...
  defmodule ObjectiveResponse do
    ...
    OpenApiSpex.schema(%{
      ...
      properties: %{
        ...
        parentId: %Schema{type: :integer, description: "The id of the parent objective, if this is not a top-level objective"}
      },
  ...
  responses: %{
    200 => {"All objectives", "application/json", OliWeb.ObjectivesController.ObjectiveResponse}
  }
  def index(conn, _) do

    objectives = conn.assigns[:project]
  ...
  @doc parameters: [
    project: [in: :url, schema: %OpenApiSpex.Schema{type: :string}, required: true, description: "The project id"],
    objective: [in: :url, schema: %OpenApiSpex.Schema{type: :string}, required: true, description: "The objective id"]
  ],
  request_body: {"Attributes for updating an objective", "application/json", ObjectiveUpdateAttributes, required: true},
  responses: %{
  ...
  def update(conn, %{"objective" => objective_id}) do
    ...
    case conn.body_params["title"] do
      ...
      nil -> error(conn, 400, "Missing title")

      title ->
        with {:ok, %{slug: slug}} <- AuthoringResolver.from_resource_id(project.slug, objective_id) |> Oli.Utils.trap_nil(),
          {:ok, _} <- ObjectiveEditor.edit(slug, %{title: title}, author, project)
        do
  ...
  defmodule ObjectiveCreationAttributes do
    ...
    OpenApiSpex.schema(%{
      ...
      properties: %{
        title: %Schema{type: :string, description: "Title of this objective"},
        parentId: %Schema{type: :integer, description: "The parent objective to attach to, omit if top-level"}
      },
      required: [:title],
      example: %{
        "title" => "Learn to do a new skill"
  ...
  request_body: {"Attributes for creating an objective", "application/json", ObjectiveCreationAttributes, required: true},
  responses: %{
  ...
  def create(conn, _) do
    ...
    parent_slug = case Map.get(conn.body_params, "parentId") do
      nil -> nil
      id -> case AuthoringResolver.from_resource_id(project.slug, id) do
        nil -> nil
        rev -> rev.slug
      end
    end
