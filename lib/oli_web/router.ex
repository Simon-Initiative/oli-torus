defmodule OliWeb.Router do
  ...
  # Legacy authoring project routes kept for backward compatibility
  scope "/authoring", OliWeb.Workspaces do
    pipe_through([:browser, :authoring_protected, :workspace])

    live_session :legacy_authoring_index,
      root_layout: {OliWeb.LayoutView, :delivery},
      layout: {OliWeb.Layouts, :workspace},
      on_mount: [
        {OliWeb.AuthorAuth, :ensure_authenticated},
      ...
      live("/projects", CourseAuthor.IndexLive)
    end
  end

  scope "/authoring/project", OliWeb.Workspaces do
    pipe_through([:browser, :authoring_protected, :workspace, :authorize_project])

    live_session :legacy_authoring_project,
      root_layout: {OliWeb.LayoutView, :delivery},
      layout: {OliWeb.Layouts, :workspace},
      on_mount: [
        {OliWeb.AuthorAuth, :ensure_authenticated},
      ...
      live("/:project_id/publish", CourseAuthor.PublishLive)
      live("/:project_id/overview", CourseAuthor.OverviewLive)
      live("/:project_id/curriculum", CourseAuthor.CurriculumLive)
      ...
      live("/:project_id/review", CourseAuthor.ReviewLive)
      live("/:project_id/alternatives", CourseAuthor.AlternativesLive)
      live("/:project_id/objectives", CourseAuthor.ObjectivesLive)
      live("/:project_id/experiments", CourseAuthor.ExperimentsLive)
      live("/:project_id/bibliography", CourseAuthor.BibliographyLive)
      live("/:project_id/index_csv", CourseAuthor.IndexCsvLive)
      live("/:project_id/index", CourseAuthor.IndexLive)
      live("/:project_id/datasets", CourseAuthor.DatasetsLive)
      live("/:project_id/datasets/create", CourseAuthor.CreateJobLive)
      live("/:project_id/datasets/details/:job_id", CourseAuthor.DatasetDetailsLive)

      scope "/:project_id/products" do
        live("/", CourseAuthor.ProductsLive)
        live("/:product_id", CourseAuthor.Products.DetailsLive)
        live("/:product_id/payments", CourseAuthor.Products.PaymentsLive)
      end
    end
  end
