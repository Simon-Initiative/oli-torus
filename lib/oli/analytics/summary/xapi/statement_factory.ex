defmodule Oli.Analytics.Summary.XAPI.StatementFactory do
  ...
  def to_statements(%AttemptGroup{
    ...
    parts_and_activities = Enum.map(part_attempts, fn part_attempt -> PartAttemptEvaluated.new(context, part_attempt, resource_attempt) end)
    ++ Enum.map(activity_attempts, fn activity_attempt -> ActivityAttemptEvaluated.new(context, activity_attempt, resource_attempt) end)
