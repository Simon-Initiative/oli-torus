defmodule Oli.Delivery.Depot.MatchSpecTranslator do
  ...
  def translate(field_types, key, conditions, fields_to_return) do
    ...
    conditions_by_field = Enum.reduce(conditions, %{}, fn {key, _} = c, acc ->
      Map.put(acc, key, c)
    end)

    return_fields = Enum.reduce(fields_to_return, MapSet.new(), fn key, acc ->
      MapSet.put(acc, key)
    end)

    {m, c, f, _} = Enum.reduce(field_types, {[], [], %{}, 0}, fn {field_name, type}, {m, c, f, v} ->
      ...
      {m, v} = if in_either do {[field(v + 1) | m], v + 1} else {[:_ | m], v} end

      {m, c, v} = if in_condition do handle_cond(type, condition, in_return, {m, c, v}) else {m, c, v} end

      f = if in_return do Map.put(f, field_name, field(v)) else f end
  ...
  defp handle_cond(type, {_f, {:between, value1, value2}}, _, {m, c, v}) do
    {m , [{:andalso, {:>, field(v), encode(value1, type)}, {:<, field(v), encode(value2, type)}} | c], v}
  end
