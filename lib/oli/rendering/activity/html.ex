defmodule Oli.Rendering.Activity.Html do
  ...
  def activity(%Context{activity_map: activity_map, render_opts: render_opts, preview: preview, progress_state: progress_state} = context, %{"activity_id" => activity_id, "purpose" => purpose} = activity) do
    ...
    case activity_summary do
      ...
      _ ->
        ...
        activity_html = ["<#{tag} class=\"activity-container\" graded=\"#{graded}\" state=\"#{state}\" model=\"#{model_json}\" preview=\"#{preview}\" progress_state=\"#{progress_state}\" section_slug=\"#{section_slug}\"></#{tag}>\n"]
