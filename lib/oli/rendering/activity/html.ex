      ...
      _ ->
        ...
        activity_html = [
          "<#{tag} class=\"activity-container\" graded=\"#{graded}\" state=\"#{state}\" model=\"#{
            model_json
          }\" preview=\"#{preview}\" progress_state=\"#{progress_state}\"></#{tag}>\n"
        ]
  ...
  def error(%Context{}, _activity, error) do
    case error do
      {:invalid, error_id, _error_msg} ->
        [
          "<div class=\"activity invalid alert alert-danger\">Activity is invalid. Please contact support with issue <strong>##{
            error_id
          }</strong></div>\n"
        ]
