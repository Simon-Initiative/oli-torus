defmodule Oli.Authoring.Clone do
  ...
  def clone_project(project_slug, author) do
    Repo.transaction(fn ->
      with {:ok, base_project} <- Course.get_project_by_slug(project_slug) |> Repo.preload(:family) |> trap_nil(),
          {:ok, cloned_family} <- Course.create_family(%{
            title: base_project.family.title <> " Copy",
            description: base_project.family.description
          }),
          {:ok, cloned_project} <- Course.create_project(%{
  ...
  def clone_all_project_resources(base_project_id, cloned_project_id) do
    ...
    |> Enum.map(&Repo.insert!(Course.change_project_resource(%ProjectResource{}, %{
  ...
  def clone_all_media_items(base_project_slug, cloned_project_id) do
    ...
    media_items
    |> Enum.map(&Repo.insert!(MediaLibrary.change_media_item(%MediaItem{}, %{
      url: &1.url,
