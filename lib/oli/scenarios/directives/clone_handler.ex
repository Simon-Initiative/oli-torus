      ...
      if source == nil do
        ...
        case Clone.clone_project(source.project.slug, state.current_author) do
          {:ok, cloned_project} ->
            ...
            cloned_project = if title do
              {:ok, updated_project} = Oli.Authoring.Course.update_project(cloned_project, %{title: title})
              updated_project
            else
              cloned_project
            end
            ...
            built_project = %Oli.Scenarios.Types.BuiltProject{
              ...
              root: %{
                resource: nil,  # We don't need the resource for clone
                revision: root_revision,
                author: state.current_author
              },
              id_by_title: id_by_title,
              rev_by_title: Map.put(rev_by_title, "root", root_revision),
              objectives_by_title: %{},  # For now, leave empty
              tags_by_title: %{}  # For now, leave empty
            }
