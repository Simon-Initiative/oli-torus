defmodule Oli.Scenarios.Directives.HookHandler do
  ...
  defp parse_function_spec(spec) when is_binary(spec) do
    ...
    case Regex.run(~r/^(.+)\.([^\/]+)\/(\d+)$/, spec) do
      ...
      _ ->
        {:error, "Invalid function specification format. Expected 'Module.function/arity', got: #{spec}"}
    end
  end
  ...
  defp compile_from_scenarios(relative_path) do
    ...
    Enum.find_value(possible_paths, {:error, "File not found in any expected location"}, fn path ->
