defmodule ECL.Constellation do
  ...
  def get_default_notebook_id(auth_token) do
    ...
    financingTeamId =
      download(auth_token, meId, ["FinancingTeams"])
      |> Map.get("responses")
      |> List.first()
      |> Map.get("fields")
    ...
    defaultNotebookId =
      download(auth_token, financingTeamId, ["DefaultNotebook"])
      |> Map.get("responses")
      |> List.first()
      |> Map.get("fields")
  ...
  def execute_sll_expression(auth_token, mm_expression) do
    ...
    responseObject =
      search(
        auth_token,
        "Object.Software.ManifoldKernel",
        "Available = true AND ManifoldJob->Computations->Status=\"Running\""
      )
    ...
    command_upload_response =
      upload(auth_token, "Object.Software.ManifoldKernelCommand", "", command_upload_data)
  ...
  def wait_for_command_to_complete(auth_token, command_id, cas) do
    ...
    download_response =
      download(auth_token, command_id, ["Status", "Result"])
      |> Map.get("responses")
      |> List.first()
  ...
  def poll_object_change(auth_token, object_id, cas) do
    ...
    pollResponse =
      send_constellation_request_with_retries(long_polling_path, request_body, auth_token, "POST")
