defmodule Oli.GoogleDocs.Client do
  ...
  @default_headers [
    ...
    {"user-agent", "TorusGoogleDocsImport/1.0"}
  ]
  ...
  defp build_export_url(file_id), do: "#{@export_url}/#{file_id}/export?format=md"

  defp request(url, file_id, opts) do
    max_attempts = opts |> Keyword.get(:max_attempts, @default_max_attempts) |> max(1)
    do_request(url, file_id, opts, 1, max_attempts)
  end

  defp do_request(url, file_id, opts, attempt, max_attempts) do
    case Oli.HTTP.http().get(url, request_headers(opts), request_options(opts)) do
      {:ok, %Response{} = response} ->
        if success_status?(response.status_code) do
          {:ok, response}
        else
          handle_unsuccessful_response(response, url, file_id, opts, attempt, max_attempts)
        end

      {:error, reason} ->
        handle_error_response(reason, url, file_id, opts, attempt, max_attempts)
    end
  end
  ...
  defp handle_unsuccessful_response(
    ...
    if retriable_status?(response.status_code) and attempt < max_attempts do
      backoff(attempt, opts)
      do_request(url, file_id, opts, attempt + 1, max_attempts)
    else
      {:error, {:http_status, response.status_code, redact_body(response)}}
    end
  end

  defp handle_error_response(reason, url, file_id, opts, attempt, max_attempts) do
  ...
  defp enforce_success(%Response{status_code: status} = response, _file_id, _opts) do
  ...
  defp request_options(opts) do
    ...
    hackney_opts = opts |> Keyword.get(:hackney, []) |> Keyword.put_new(:pool, @hackney_pool)
