defmodule Oli.GoogleDocs.MarkdownParser do
  ...
  defp build_result(ast, messages, metadata, max_nodes, opts \\ []) do
    warnings = Enum.map(messages, &earmark_message_to_warning/1)
  ...
  defp extract_custom_element(children) do
    with {"thead", _, [{"tr", _, header_cells, _}], _} <-
           Enum.find(children, &match?({"thead", _, _, _}, &1)),
         [first_header, second_header | _] <- header_cells,
         "CustomElement" <- extract_plain_text(elem(first_header, 2)),
         type when type != "" <- extract_plain_text(elem(second_header, 2)) do
      rows =
        children
        ...
        |> Enum.map(fn
          {"tr", _, [key_cell, value_cell], _} ->
            key = extract_plain_text(elem(key_cell, 2)) |> String.trim()
            value = extract_plain_text(elem(value_cell, 2)) |> String.trim()
            {key, value}

          {"tr", _, cells, _} when length(cells) >= 2 ->
            key = extract_plain_text(elem(Enum.at(cells, 0), 2)) |> String.trim()
            value = extract_plain_text(elem(Enum.at(cells, 1), 2)) |> String.trim()
            {key, value}
  ...
  defp parse_inline_element(text, marks) when is_binary(text) do
    if text == "" do
      ...
      [%{text: text, marks: marks, href: nil}]
    end
  end

  defp parse_inline_element({"strong", _attrs, children, _meta}, marks) do
    parse_inlines(children, [:bold | marks])
  end
  ...
  defp parse_inline_element({"em", _attrs, children, _meta}, marks) do
    parse_inlines(children, [:italic | marks])
  end
