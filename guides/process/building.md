...
Mix environment files are compile-time configuration. `config/runtime.exs` supplies deployment-specific values when a release starts. Building with `MIX_ENV=preview` makes `bin/seed` available and compiles the web QA features into the artifact. Running `bin/seed` is an explicit low-level operation and needs no runtime feature flag. To activate web-accessible masquerade and mailbox features, set the runtime variable below to the exact value `true`, ignoring letter case:
