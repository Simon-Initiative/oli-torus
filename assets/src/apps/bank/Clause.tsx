...
export const Clause: React.FC<ClauseProps> = (props: ClauseProps) => {
  ...
  const children = clause.children.map((c, i) => {
    if (c.operator === Bank.ClauseOperator.all || c.operator === Bank.ClauseOperator.any) {
      ...
      return <Expression
        {...props}
        expression={c as Bank.Expression}
        onChange={e => {
          const index = i;
          onChange(Object.assign({}, clause, { children: replaceAt(clause.children, index, e) }));
        }}
  ...
  return (
    <div>
      <div className="btn-group btn-group-toggle" data-toggle="buttons">
        <label className={`btn btn-secondary ${clause.operator === Bank.ClauseOperator.all ? 'active' : ''}`}>
          <input
            onChange={() => {
        ...
        <label className={`btn btn-secondary ${clause.operator === Bank.ClauseOperator.any ? 'active' : ''}`}>
          <input
            onChange={() => {
