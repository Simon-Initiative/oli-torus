...
type AITriggerIconProps = Omit<React.ComponentPropsWithoutRef<'svg'>, 'fill'> & {
  stroke?: string;
  fill?: string;
  title?: string;
};

export const AITriggerIcon: React.FC<AITriggerIconProps> = ({
  stroke = 'currentColor',
  fill = 'currentColor',
  title,
  ...props
}) => {
  const hasAccessibleName = !!(title || props['aria-label'] || props['aria-labelledby']);
