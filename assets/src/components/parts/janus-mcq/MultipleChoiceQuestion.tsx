  ...
  return (
      <div>
          {nodes.map((subtree: any) => {
              ...
              if (subtree.tag === 'p') {
                  // PMP-347
                  const hasImages = subtree.children.some(
                      (child: { tag: string; }) => child.tag === 'img'
                  );
                  if (hasImages) {
...
const MCQItem: React.FC<any> = ({
  ...
  if (layoutType === 'horizontalLayout') {
      ...
      const hasBlankSpans = nodes.some((node: any) =>
          node.children.some(
              (child: { tag: string; children: string | any[]; }) => child.tag === 'span' && child.children.length === 0
          )
      );
    ...
    if (
        ...
        ((prevSelectedChoices.length < 1 && selectedChoices.length > 0) ||
            ...
            (prevSelectedChoices.length > 0 &&
                !prevSelectedChoices.every((fact) =>
                    selectedChoices.includes(fact)
                )))
    ) {
...
const handleItemSelection = ({ value, textValue, checked }:{ value:number, textValue:string, checked:boolean }) => {
    ...
    if (!multipleSelection) {
        ...
        newSelectedChoices = [
            ...new Set([...selectedChoices, newChoice]),
        ].filter((c) => checked || (!checked && newChoice !== c));

        const updatedSelections = selectedChoicesText.map((item) => {
        const modifiedItem = { ...item };
            modifiedItem.checked =
                item.value === value ? checked : item.checked;
            return modifiedItem;
        });
