...
export const InviteStudentsPopup = (props: Props): JSX.Element => {
  const [show, setShow] = useState(false);

  const onDismiss = () => {
    setShow(false);
    sessionStorage.setItem('inviteStudentsPrompt', 'hide');
  };

  const onDontAskAgain = () => {
    setShow(false);
    localStorage.setItem('inviteStudentsPrompt', 'hide');
  };

  const onBlurClick = (e: any) => {
    if (!e.InviteStudentsPopupClick) {
      setShow(false);
  ...
  useEffect(() => {
    const showPref =
      sessionStorage.getItem('inviteStudentsPrompt') != 'hide' &&
      localStorage.getItem('inviteStudentsPrompt') != 'hide';
    setShow(showPref);
    window.addEventListener('click', onBlurClick);
  }, []);

  const onClick = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    (e.nativeEvent as any).InviteStudentsPopupClick = true;
  };
