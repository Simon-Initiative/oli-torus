...
export const onPaste = async (editor: SlateEditor,
  e: React.ClipboardEvent<HTMLDivElement>, projectSlug: string) => {
  ...
  return uploadFiles(projectSlug, files)
    .then(uploadedFiles => uploadedFiles
      .map((file: any) => file.url)
      .filter((url: string | undefined) => !!url))
    .then(urls => urls.forEach((url: string) =>
