...
export const RulesTab: React.FC<Props> = (props) => {
  const { model, projectSlug, dispatch } = useAuthoringElementContext<MultiInputSchema>();

  const removeInputFromResponse = () => {
    dispatch(MultiInputActions.removeInputFromResponse(props.input.id, props.response.id));
  };
