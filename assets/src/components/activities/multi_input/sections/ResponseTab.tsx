...
export const ResponseTab: React.FC<Props> = (props) => {
  ...
  const { model, dispatch, authoringContext, editMode } = useAuthoringElementContext<MultiInputSchema>();
  ...
  const getResponseBody = () => {
    return inputs ? (
      inputs.map((i) => (
        <RulesTab key={i.id} input={i} response={{ ...response, rule: inputRules[i.id] }} toggleCorrectness={toggleCorrectness} editRule={editRule}/>
      ))
    ) : null;
  };
    ...
    <Card.Content>
      ...
      <FeedbackCard
        ...
        updateTextDirection={(textDirection) =>
          updateTextDirection(response.id, textDirection)
        }
