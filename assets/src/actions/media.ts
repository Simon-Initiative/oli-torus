...
export type MediaActions = FetchMediaPageAction | ReceiveMediaPageAction
  | ResetMediaAction | LoadMediaReferencesAction | SideloadDataAction;
...
export const fetchMediaPage = (courseId: ProjectSlug, reqId: string):
...
export const receiveMediaPage = (
  courseId: ProjectSlug, items: List<MediaItem>, totalItems: number):
...
export const sideloadData = (
  courseId: ProjectSlug, data: Map<string, MediaItem>): SideloadDataAction => ({
    type: SIDELOAD_DATA,
    courseId,
    data,
  });
...
export const loadMediaReferences = (
  courseId: ProjectSlug, references: Map<string, List<MediaRef>>):
...
export const fetchCourseMedia = (
  courseId: ProjectSlug, offset?: number, limit?: number, mimeFilter?: string[],
  searchText?: string, orderBy?: string, order?: string) => (
    (dispatch: Dispatch, getState: () => State): Promise<Maybe<List<MediaItem>>> => {
      ...
      return persistence.fetchMedia(
        courseId, offset, limit, mimeFilter, undefined, searchText, orderBy, order)
        .then((response) => {
...
export const fetchCourseMediaNextPage = (
  courseId: ProjectSlug, mimeFilter?: string[], searchText?: string,
