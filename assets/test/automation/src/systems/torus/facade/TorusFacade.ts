...
import { AdminManagmentPO } from "../pom/workspace/administrator/AdminManagmentPO";
...
export class TorusFacade {
  ...
  private adminManagment: AdminManagmentPO;
  ...
  constructor(page: Page, environment?: string) {
    ...
    this.adminManagment = new AdminManagmentPO(this.page);
  ...
  async createNewProjectAsOpen(projectNameFilter: string) {
    ...
    await this.wsa.getSidebar().publishProject();
  ...
  async canCreateSections(searchEmail: string, nameLink: string) {
    ...
    await this.adminManagment.clickManageStudentsInstructorAccounts();
    await this.adminAllUsers.searchUser(searchEmail);
    await this.adminAllUsers.clickNameLink(nameLink);
