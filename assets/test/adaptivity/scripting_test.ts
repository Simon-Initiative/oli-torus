...
describe('Scripting Interface', () => {
  ...
  describe('variable helper methods', () => {
    it('should detect prefixes properly', () => {
      const expr1 = '{app.spr-app-id.data key}';
      const expr2 = '{app.spr-app.id.data key}';

      expect(containsExactlyOneVariable(expr1)).toBe(true);
      expect(variableContainsValidPrefix(expr1)).toBe(true);

      expect(containsExactlyOneVariable(expr2)).toBe(false);
      expect(variableContainsValidPrefix(expr2)).toBe(true);
    });
  });
});
