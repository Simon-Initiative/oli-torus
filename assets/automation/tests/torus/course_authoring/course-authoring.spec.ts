...
test.describe('Course authoring', () => {
  test('Author publishes project, admin makes it public, instructor creates a section', async ({
    homeTask,
    projectTask,
    curriculumTask,
    utils,
  ...
  test.describe('Add one of each type of content to dedicated pages', () => {
    ...
    const pages = [
      {
        title: 'Cite',
        action: async (ct: any) => ct.addCiteToolbar('Newton, I.', 'Newton', false),
        verify: async (p: any) => p.verifyCite('Newton'),
      },
      {
        title: 'Foreign',
        action: async (ct: any) =>
          ct.addForeignToolbar('Text generated by automation', 'arabic', false),
        verify: async (p: any) => p.verifyTextAnywhere('Text generated by automation'),
      },
      {
        title: 'Image',
        action: async (ct: any) => ct.addImageToolbar('img-mock-05-16-2025.jpg', false),
        verify: async (p: any) => p.verifyMedia('img-mock-05-16-2025.jpg', 'img'),
      },
      {
        title: 'Formula',
        action: async (ct: any) => ct.addFormulaToolbar('1+2=3', false),
        verify: async (p: any) => p.verifyTextAnywhere('1+2=3'),
      },
      {
        title: 'Callout',
        action: async (ct: any) =>
      ...
      },
      {
        title: 'Popup',
        action: async (ct: any) =>
          ct.addPopUpToolbar('Text generated by automation', 'Popup text', false),
        verify: async (p: any) => {
      ...
      },
      {
        title: 'Dialog',
        action: async (ct: any) =>
          ct.addDialogToolbar("This is a dialog's title", 'Leonardo', 'This is my text', false),
        verify: async (p: any) => p.verifyTextAnywhere("This is a dialog's title"),
      },
      {
        title: 'Table',
        action: async (ct: any) =>
          ct.addTableToolbar('My table Caption', 'My cell 1', 'My cell 2', false),
        verify: async (p: any) => p.verifyTextAnywhere('My cell 1'),
      },
      {
        title: 'Theorem',
        action: async (ct: any) => ct.addTheoremToolbar('Theorem Title', false),
        verify: async (p: any) => p.verifyTextAnywhere('Theorem Title'),
      },
      {
        title: 'Conjugation',
        action: async (ct: any) =>
          ct.addConjugationToolbar(
            'Singular',
            'Plural',
            '1st Person',
            '2nd Person',
      ...
      },
      {
        ...
        action: async (ct: any) =>
          ct.addDescriptionListToolbar('My title', 'My term', 'My definition', false),
        verify: async (p: any) => p.verifyTextAnywhere('My definition'),
      },
      {
        title: 'Audio',
        action: async (ct: any) =>
          ct.addAudioClipToolbar('audio-test-01.mp3', 'My Audio Caption', false),
        verify: async (p: any) => p.verifyMedia('audio-test-01.mp3', 'audio'),
      },
      {
        title: 'Video',
        action: async (ct: any) => ct.addVideoToolbar('video-test-01.mp4', false),
        verify: async (p: any) => p.verifyMedia('video-test-01.mp4', 'video'),
      },
      {
        title: 'YouTube',
        action: async (ct: any) =>
          ct.addYoutubeToolbar('https://www.youtube.com/watch?v=2QAMzupR_C4', '2QAMzupR_C4', false),
        verify: async (p: any) => p.verifyMedia('2QAMzupR_C4', 'youtube'),
      },
      {
        title: 'Webpage',
        action: async (ct: any) =>
          ct.addWebPageToolbar('https://es.wikipedia.org/wiki/Wikipedia:Portada', false),
        verify: async (p: any) =>
          p.verifyMedia('es.wikipedia.org/wiki/Wikipedia:Portada', 'webpage'),
      },
      {
        title: 'Code Block',
        action: async (ct: any) =>
          ct.addCodeBlockToolbar('python', 'print("Hola, mundo!")', 'Test Code block', false),
        verify: async (p: any) => p.verifyTextAnywhere('Hola, mundo'),
      },
      {
        title: 'Figure',
        action: async (ct: any) => ct.addFigureToolbar('Test Figure Title', false),
        verify: async (p: any) => p.verifyTextAnywhere('Test Figure Title'),
      },
      {
        title: 'Page Link',
        action: async (ct: any) => ct.addPageLinkToolbar('Page 1', false),
        verify: async (p: any) => p.verifyTextAnywhere('Page 1'),
      },
      {
        title: 'Definition',
        action: async (ct: any) =>
  ...
  test('Author publishes project, admin makes it public, instructor creates section', async ({
    homeTask,
    projectTask,
    curriculumTask,
    utils,
