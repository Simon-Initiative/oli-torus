...
/**
 * 's setup-response anchor: the frozen triple's section must equal the
 ...
 * is caught only here. Lives OUTSIDE the driver on purpose: it is the
  ...
  if (frozen?.sectionSlug !== expectedSectionSlug) {
    throw new Error(
      `correlation section "${String(frozen?.sectionSlug)}" does not match the setup ` +
        `response "${expectedSectionSlug}" (setup-response anchor)`,
    );
  }
}

/**
 * Total over unknown thrown values: a promise may
