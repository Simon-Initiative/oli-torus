 ...
 * @param defaultValue
 */
export const valueOr = (value, defaultValue) => value === null || value === undefined ? defaultValue : value;
...
export const parseBoolean = (input) => input !== undefined &&
    (input === true ||
        input === 1 ||
        input.toString().toLowerCase() === 'true' ||
        input.toString().toLowerCase() === 'on' ||
        input.toString().toLowerCase() === '1');
export const isStringArray = (s) => typeof s === 'string' && s.charAt(0) === '[' && s.charAt(s.length - 1) === ']';
...
export const parseArray = (val) => {
    if (Array.isArray(val)) {
        return val.map((item) => parseNumString(item));
    }
    if (isStringArray(val)) {
        ...
        if (isNested) {
            ...
            const elements = inner.split(',').map((item) => parseNumString(item));
    ...
    if (!val) {
        ...
        return val.split(',').map((item) => parseNumString(item));
    }
...
export const parseNumString = (item) => {
...
export const zip = (xs1, xs2) => xs1.reduce((acc, x, i) => (i > xs2.length - 1 ? acc : acc.concat([[x, xs2[i]]])), []);
