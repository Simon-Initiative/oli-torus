...
function convertHour(hour, isPM) {
    if (isPM) {
...
export function parseDate(value) {
    const p = value.split(' ');
    const t = p[3].split(':');
    return new Date(Date.UTC(parseInt(p[2], 10), monthsToOrdinal[p[0]], parseInt(p[1].substr(0, p[1].indexOf(',')), 10), convertHour(parseInt(t[0], 10), p[4] === 'PM'), parseInt(t[1], 10), parseInt(t[2], 10)));
}
const dateOptions = {
    month: 'long',
    day: 'numeric',
...
export const dateFormatted = (date) => date.toLocaleDateString('en-US', dateOptions);
export function compareDates(a, b) {
    return a.valueOf() - b.valueOf();
}
export function relativeToNow(a) {
    return relativeTo(a, new Date());
}
// Take a date and return a new date taking into account
// some amount of time skew
export function adjustForSkew(a, skewInMs) {
 ...
 */
export function relativeTo(dateFrom, dateNow) {
