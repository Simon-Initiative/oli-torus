...
const scrollToResourceEditor = (contentId) => {
    setTimeout(() => {
        var _a;
        (_a = document.querySelector(`#re${contentId}`)) === null || _a === void 0 ? void 0 : _a.scrollIntoView({ behavior: 'smooth' });
    });
};
export const dropHandler = (content, onEditContentList, projectSlug, onDragEnd, editMode) => (e, index) => {
    onDragEnd();
    if (editMode) {
            ...
            if (sourceIndex === -1) {
                ...
                if (droppedContent.type === 'ActivityPayload') {
                    if (droppedContent.project !== projectSlug) {
                        Persistence.create(droppedContent.project, droppedContent.activity.typeSlug, droppedContent.activity.model, []).then((result) => {
