...
export const Objectives = (props) => {
    ...
    const map = Immutable.Map(objectives.map((o) => [o.id, o]));
    const asObjectives = selected.map((s) => map.get(s));
    return (<div className="flex-grow-1 objectives">
      <Typeahead id={id} multiple={true} disabled={!editMode} onChange={(updated) => {
        ...
        }} options={props.objectives} allowNew={true} newSelectionPrefix="Create new objective: " selectHintOnEnter={true} labelKey="title" selected={asObjectives} placeholder="Attach learning objectives..."/>
    </div>);
};
//# sourceMappingURL=Objectives.jsx.map