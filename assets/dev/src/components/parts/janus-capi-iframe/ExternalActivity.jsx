...
const externalActivityMap = new Map();
let context = 'VIEWER';
...
const ExternalActivity = (props) => {
    ...
    const [frameZ, setFrameZ] = useState(0);
    ...
    useEffect(() => {
        ...
        if (frameWidth !== undefined) {
            styleChanges.width = { value: frameWidth };
        }
        if (frameHeight != undefined) {
            styleChanges.height = { value: frameHeight };
        }
    ...
    const processInitStateVariable = (currentStateSnapshot, domain = 'stage') => {
        ...
        const interestedSnapshot = Object.keys(currentStateSnapshot).reduce((collect, key) => {
    ...
    const sendToIframe = (data) => {
        var _a;
        (_a = simFrame === null || simFrame === void 0 ? void 0 : simFrame.contentWindow) === null || _a === void 0 ? void 0 : _a.postMessage(JSON.stringify(data), '*');
    };
    const writeCapiLog = (msg, ...rest) => {
                    ...
                    case NotificationType.CHECK_STARTED:
                        {
                            ...
                            sendFormedResponse(simLife.handshake, {}, JanusCAPIRequestTypes.CHECK_START_RESPONSE, {});
                        }
                        break;
                    case NotificationType.CHECK_COMPLETE:
                        {
                            ...
                            sendFormedResponse(simLife.handshake, {}, JanusCAPIRequestTypes.CHECK_COMPLETE_RESPONSE, {});
                        }
                        break;
                    case NotificationType.STATE_CHANGED:
                        {
                            ...
                            Object.keys(currentMutateStateSnapshot).forEach((key) => {
                                externalActivityMap.set(`${simLife.ownerActivityId}|${key}`, currentMutateStateSnapshot[key]);
                            });
    ...
    const updateInternalState = (vars) => {
    ...
    const createCapiObjectFromStateVars = (vars, domain = 'stage') => {
        return vars
    ...
    const sendFormedResponse = (handshake, options, type, values) => {
        const responseMsg = {
            handshake,
            options,
            type,
    ...
    const handleHandshakeRequest = (msgData) => {
        const { handshake: { requestToken: msgRequestToken }, } = msgData;
    ...
    const handleOnReady = (data) => {
        ...
        simLife.ready = true;
        const updateSimLife = Object.assign({}, simLife);
        ...
        if (filterVars && ((_a = Object.keys(filterVars)) === null || _a === void 0 ? void 0 : _a.length) !== 0) {
        ...
        if (!initState && !((_b = Object.keys(initState)) === null || _b === void 0 ? void 0 : _b.length)) {
            simLife.init = true;
            sendFormedResponse(simLife.handshake, {}, JanusCAPIRequestTypes.INITIAL_SETUP_COMPLETE, {});
        }
        return;
    };
    const handleGetData = (msgData) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            ...
            const val = yield props.onGetData({ simId, key, id });
            let value = val;
        ...
        sendFormedResponse(simLife.handshake, msgData.options, JanusCAPIRequestTypes.GET_DATA_RESPONSE, response.values);
    });
    const debounceSave = useCallback(debounce(({ id, responses }) => {
        props.onSave({
            id,
            responses,
        });
    }, 100, { maxWait: 30000, leading: true }), []);
    const handleValueChange = (msgData, domain = 'stage') => {
    ...
    const handleSetData = (message) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            ...
            const obj = value;
            yield props.onSetData({ simId, key, value: obj, id });
            response.values.responseType = 'success';
            response.values.value = value;
        }
        catch (err) {
        ...
        sendFormedResponse(simLife.handshake, message.options, JanusCAPIRequestTypes.SET_DATA_RESPONSE, response.values);
    });
    const handleCheckRequest = (data) => {
        setTimeout(() => {
            props.onSubmit({
                id: `${id}`,
                responses: [],
    ...
    const handleResizeParentContainer = useCallback((data) => {
        var _a, _b, _c, _d;
        ...
        if ((_a = modifiedData === null || modifiedData === void 0 ? void 0 : modifiedData.height) === null || _a === void 0 ? void 0 : _a.value) {
            iFrameResponse.push({
                key: `IFRAME_frameHeight`,
                type: CapiVariableTypes.NUMBER,
                value: ((_b = modifiedData === null || modifiedData === void 0 ? void 0 : modifiedData.height) === null || _b === void 0 ? void 0 : _b.value) || frameHeight,
            });
        }
        if ((_c = modifiedData === null || modifiedData === void 0 ? void 0 : modifiedData.width) === null || _c === void 0 ? void 0 : _c.value) {
            iFrameResponse.push({
                key: `IFRAME_frameWidth`,
                type: CapiVariableTypes.NUMBER,
                value: ((_d = modifiedData === null || modifiedData === void 0 ? void 0 : modifiedData.width) === null || _d === void 0 ? void 0 : _d.value) || frameWidth,
            });
        }
        props.onSave({
            id,
        ...
        sendFormedResponse(simLife.handshake, {}, JanusCAPIRequestTypes.RESIZE_PARENT_CONTAINER_RESPONSE, {
            messageId: data.messageId,
            responseType: 'success',
        });
    }, [frameWidth, frameHeight]);
    //#endregion
    const handleIFrameSpecificProperties = (stateVars, domain = 'stage') => {
    ...
    useEffect(() => {
        ...
        const configDataState = [
            ...configData.map((cdVar) => {
                return Object.assign(Object.assign({}, cdVar), { id: `${newLife.domain}.${id}.${cdVar.key}` });
            }),
        ];
        // override configData values from init trap state data.
        const newInternalState = (configDataState === null || configDataState === void 0 ? void 0 : configDataState.map((item) => {
            var _a;
            const initStateValue = initState[item.id];
            if ((initStateValue === null || initStateValue === void 0 ? void 0 : initStateValue.length) || ((_a = initStateValue === null || initStateValue === void 0 ? void 0 : initStateValue.toString()) === null || _a === void 0 ? void 0 : _a.length)) {
        ...
        const messageListenerRef = window.addEventListener('message', (evnt) => {
            if (!((simFrame === null || simFrame === void 0 ? void 0 : simFrame.contentWindow) === evnt.source)) {
                return;
            }
            let data;
            try {
    ...
    useEffect(() => {
        ...
        Object.keys(initState)
            ...
            if (baseKey.indexOf('System.AllowNextOnCacheCase') !== -1) {
                const mFormatted = {};
                const updatedVar = Object.assign({}, cVar);
    ...
    return initStateReceived ? (<iframe data-janus-type={tagName} ref={frameRef} style={externalActivityStyles} title={title} src={frameSrc} scrolling={scrolling}/>) : null;
};
export const tagName = 'janus-capi-iframe';
export default ExternalActivity;
//# sourceMappingURL=ExternalActivity.jsx.map