...
const CapiIframeAuthor = (props) => {
    ...
    const configDivStyles = {
        padding: 0,
        margin: 0,
    };
    const configAnchorStyles = {
        padding: 0,
        margin: 0,
        textDecoration: 'none',
        borderBottom: 1,
    ...
    const handleValueChange = (msgData) => {
    ...
    const handleOnReady = (data) => {
        ...
        simLife.currentState.forEach((variable) => {
            const collect = {};
    ...
    useEffect(() => {
        ...
        setSimLife(newLife);
        const configDataState = configData
            ? [
                ...configData === null || configData === void 0 ? void 0 : configData.map((cdVar) => {
                    return Object.assign(Object.assign({}, cdVar), { id: `stage.${id}.${cdVar.key}` });
                }),
            ]
            : [];
    ...
    const handleValueChangeFromModal = (changedVar) => {
        var _a;
        /* console.log('CAPI: CONFIG EDITOR CHANGE', { changedVar }); */
        const finalConfigData = internalState.map((variable) => {
        ...
        const changedVariable = finalConfigData.filter((variable) => variable.key === changedVar.target);
        if (changedVar) {
            const collect = {};
            const baseKey = changedVar.target;
            const value = changedVar.value;
            const type = changedVar.type;
            const capiallowedValues = (changedVariable === null || changedVariable === void 0 ? void 0 : changedVariable.length) && ((_a = changedVariable[0]) === null || _a === void 0 ? void 0 : _a.allowedValues)
                ? changedVariable[0].allowedValues
                : undefined;
            const readonly = (changedVariable === null || changedVariable === void 0 ? void 0 : changedVariable.length) ? changedVariable[0].readonly : undefined;
            const writeonly = (changedVariable === null || changedVariable === void 0 ? void 0 : changedVariable.length) ? changedVariable[0].writeonly : undefined;
            const cVar = new CapiVariable({
                key: baseKey,
                value,
                type,
    ...
    const handleEditorSave = (changeOperations) => {
        if (!inConfigureMode) {
            return;
        }
    ...
    const configClikedRender = (<React.Fragment>
      <div style={styles}>
        {configClicked ? (<iframe ref={frameRef} style={{ height: '100%', width: '100%' }} data-janus-type={tagName} src={props.model.src} scrolling={((_a = props.type) === null || _a === void 0 ? void 0 : _a.toLowerCase()) === 'janus-capi-iframe' ? 'no' : ''}/>) : (<div className="container h-100">
            <div className="row h-100 justify-content-center align-items-center">
              <div>
                <label style={lableStyles}>{props.id}</label>
                {src && !configClicked && (<div style={configDivStyles} className="form-group">
                    {<a href="#!" onClick={() => {
                        setconfigClicked(true);
                    ...
                    <iframe ref={frameRef} style={{ display: 'none', height: '100%', width: '100%' }} data-janus-type={tagName} scrolling={((_b = props.type) === null || _b === void 0 ? void 0 : _b.toLowerCase()) === 'janus-capi-iframe' ? 'no' : ''}/>
                  </div>)}
              </div>
            </div>
          </div>)}
    ...
    const portalEl = document.getElementById(props.portal);
    const renderIt = inConfigureMode &&
        portalEl &&
        ReactDOM.createPortal(<CapiVariablePicker label="Stage" state={internalState.filter((item) => !item.readonly)} onChange={handleValueChangeFromModal} onSave={handleEditorSave} onCancel={handleEditorCancel}/>, portalEl);
...
//# sourceMappingURL=CapiIframeAuthor.jsx.map