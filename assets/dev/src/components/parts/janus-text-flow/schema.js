...
export const createSchema = (context) => {
    return {
        overrideWidth: true,
...
export const transformModelToSchema = (model) => {
    const { palette } = model;
    const paletteStyles = {
...
export const transformSchemaToModel = (schema) => {
    const { overrideHeight, overrideWidth, customCssClass, palette } = schema;
    const result = Object.assign(Object.assign({}, schema), { overrideHeight: !!overrideHeight, overrideWidth: !!overrideWidth, customCssClass: customCssClass || '' });
    if (palette) {
        result.palette = {
            useHtmlProps: true,
