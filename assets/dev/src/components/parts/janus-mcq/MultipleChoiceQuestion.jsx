...
const getNodeText = (node) => {
    ...
    nodeText += node.children.reduce((childrenText, childNode) => {
...
const MCQItemContent = ({ nodes, state }) => {
    return (
    ...
    <div style={{ left: 18, position: 'relative' }}>
      {nodes.map((subtree) => {
            ...
            if (subtree.tag === 'p') {
                // PMP-347
                const hasImages = subtree.children.some((child) => child.tag === 'img');
                if (hasImages) {
...
export const MCQItem = ({ nodes, state, multipleSelection, itemId, layoutType, totalItems, groupId, selected = false, onSelected, val, disabled, idx, overrideHeight, columns = 1, onConfigOptionClick, index, configureMode, verticalGap = 0, }) => {
    const mcqItemStyles = {};
    ...
    return (<React.Fragment>
      <div style={mcqItemStyles}>
        {configureMode && (<>
            <button className="aa-add-button btn btn-primary btn-sm" type="button" aria-describedby="button-tooltip" onClick={() => onConfigOptionClick(index, 2)} style={{
                fontSize: '10px;',
                padding: 1,
                cursor: 'pointer',
            }}>
              <i className="fa fa-trash" style={{ cursor: 'pointer', color: 'white' }} aria-hidden="true" title="Delete the option"></i>{' '}
            </button>

            <button className="aa-add-button btn btn-primary btn-sm" type="button" aria-describedby="button-tooltip" onClick={() => onConfigOptionClick(index, 1)} style={{
                fontSize: '10px;',
                padding: 1,
                marginLeft: 4,
                cursor: 'pointer',
            }}>
              <i className="fas fa-edit" style={{ cursor: 'pointer', color: 'white' }} aria-hidden="true" title="Edit the option"></i>{' '}
            </button>
            <button className="aa-add-button btn btn-primary btn-sm" type="button" aria-describedby="button-tooltip" onClick={() => onConfigOptionClick(index, 3)} style={{
                fontSize: '10px;',
                padding: 1,
                marginLeft: 4,
                cursor: 'pointer',
              ...
              <i className="fas fa-plus" style={{ cursor: 'pointer', color: 'white' }} aria-hidden="true" title="Add new option"></i>{' '}
            </button>
          </>)}
        <input style={{ position: 'absolute', marginTop: 5 }} name={groupId} id={itemId} type={multipleSelection ? 'checkbox' : 'radio'} value={val} disabled={disabled} checked={selected} onChange={handleChanged}/>
...
const getOptionTextFromNode = (children) => {
    var _a;
    let optionText = '';
    if (children.tag === 'text') {
        optionText = children.text;
    }
    else if ((_a = children === null || children === void 0 ? void 0 : children.children) === null || _a === void 0 ? void 0 : _a.length) {
        optionText = getOptionTextFromNode(children.children[0]);
    }
    return optionText;
};
const getOptionTextById = (options, optionId) => {
    const text = options
        .map((option) => {
    ...
    return (text === null || text === void 0 ? void 0 : text.length) ? text[0] : '';
};
const getOptionNumberFromText = (options, optionText) => {
...
const MultipleChoiceQuestion = (props) => {
    ...
    const initialize = useCallback((pModel) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        let mcqList = (_a = pModel.mcqItems) === null || _a === void 0 ? void 0 : _a.map((item, index) => (Object.assign(Object.assign({}, item), { index: index, value: index + 1 })));
        if (dRandomized) {
        ...
        if (!hasDoneMultiple) {
            ...
            if (sSelectedChoiceText !== undefined && !hasDoneSelectedChoice) {
                ...
                if (choiceNumber !== undefined) {
                    handleItemSelection({ value: choiceNumber, textValue: sSelectedChoiceText, checked: true }, true);
                }
            }
        }
    ...
    const { width, multipleSelection, customCssClass, layoutType, height, overrideHeight = false, } = model;
    useEffect(() => {
        let pModel;
        let pState;
    ...
    const handleMultipleItemSelection = (selections, shouldSave = true) => {
        let modifiedSelections = selections;
        const newCount = selections.length;
        const blankValueExit = (selections.length === 1 && selections.filter((item) => item.value <= 0)) || [];
    ...
    const handleItemSelection = ({ value, textValue, checked }, shouldSave = true) => {
        ...
        if (multipleSelection) {
            ...
            newSelectedChoices = [...new Set([...selectedChoices, newChoice])].filter((c) => checked || (!checked && originalValue !== c && c > 0));
        ...
        const blankValueExit = (newSelectedChoices.length === 1 && newSelectedChoices.filter((value) => value <= 0)) || [];
        if (blankValueExit.length) {
    ...
    const saveState = ({ numberOfSelectedChoices, selectedChoice, selectedChoiceText, selectedChoices, selectedChoicesText, }) => {
        props.onSave({
            id: `${id}`,
            responses: [
                {
    ...
    return ready ? (<div data-janus-type={tagName} style={styles} className={`mcq-input`}>
      {options === null || options === void 0 ? void 0 : options.map((item, index) => (<MCQItem idx={index} key={`${id}-item-${index}`} title={item.title} totalItems={options.length} layoutType={layoutType} itemId={`${id}-item-${index}`} groupId={`mcq-${id}`} selected={isItemSelected(item.index)} val={item.value} onSelected={handleItemSelection} state={state} {...item} x={0} y={0} overrideHeight={overrideHeight} disabled={!enabled} multipleSelection={multipleSelection} columns={columns}/>))}
    </div>) : null;
};
export const tagName = 'janus-mcq';
export default MultipleChoiceQuestion;
//# sourceMappingURL=MultipleChoiceQuestion.jsx.map