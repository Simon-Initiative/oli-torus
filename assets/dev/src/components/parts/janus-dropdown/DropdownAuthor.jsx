...
const DropdownAuthor = (props) => {
    ...
    const dropdownOptions = () => {
        ...
        if (optionLabels) {
            for (let i = 0; i < optionLabels.length; i++) {
                ...
                options.push(<option key={i + 1} value={i + 1} selected={i + 1 === -1}>
            {optionLabels[i]}
          </option>);
            }
        }
    ...
    return (<div data-janus-type={tagName} className="dropdown-input" style={styles}>
      <label htmlFor={`${id}-select`}>{showLabel && label ? label : ''}</label>
      <select style={dropDownStyle} id={`${id}-select`} value={-1} className={'dropdown '} disabled={true}>
        {dropdownOptions()}
      </select>
    </div>);
};
...
//# sourceMappingURL=DropdownAuthor.jsx.map