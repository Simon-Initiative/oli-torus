...
const InputTextAuthor = (props) => {
    ...
    return (<div className={`short-text-input`} style={styles}>
      <label htmlFor={`${id}-short-text-input`}>
        {showLabel && label ? label : <span>&nbsp;</span>}
      </label>
      <input name="janus-input-text" id={`${id}-short-text-input`} type="text" placeholder={prompt} disabled={true} style={{ width: '100%', fontSize }}/>
    </div>);
};
export const tagName = 'janus-input-text';
export default InputTextAuthor;
//# sourceMappingURL=InputTextAuthor.jsx.map