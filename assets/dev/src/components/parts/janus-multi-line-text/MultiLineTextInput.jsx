...
const MultiLineTextInput = (props) => {
    ...
    const { label, x = 0, y = 0, z = 0, width, height, prompt, showLabel, showCharacterCount, fontSize, } = model;
    useEffect(() => {
        if (!props.notify) {
            return;
        }
    ...
    const handleOnChange = (event) => {
        ...
        if (characterCounterRef.current)
            characterCounterRef.current.innerText = val.length.toString();
        setText(val);
    ...
    return ready ? (<div data-janus-type={tagName} className={`long-text-input`} style={wrapperStyles}>
      <label htmlFor={id} style={{
            display: showLabel ? 'inline-block' : 'none',
        }}>
        {label}
      </label>
      <textarea name="test" id={id} onChange={handleOnChange} style={inputStyles} placeholder={prompt} value={text} disabled={!enabled}/>
      <div title="Number of characters" className="characterCounter" style={{
        ...
        <span className={`span_${id}`} ref={characterCounterRef} style={{
            padding: '0px',
            fontFamily: 'Arial',
        }}>
...
//# sourceMappingURL=MultiLineTextInput.jsx.map