...
const Video = (props) => {
    const [_state, setState] = useState(Array.isArray(props.state) ? props.state : []);
    const [model, setModel] = useState(typeof props.model === 'string' ? {} : props.model);
    ...
    const initialize = useCallback((pModel) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const dEnableReplay = typeof pModel.enableReplay === 'boolean' ? pModel.enableReplay : videoEnableReplay;
    ...
    const { x, y, z, width, height, src, triggerCheck, autoPlay = false, startTime = 0, endTime = 0, enableReplay = true, subtitles, } = model;
    const videoStyles = {
    /* position: 'absolute',
    top: y,
    left: x,
    ...
    if (youtubeRegex.test(finalSrc)) {
        ...
        if (startTime && startTime >= 0) {
            youtubeOpts.playerVars = Object.assign(Object.assign({}, youtubeOpts.playerVars), { start: startTime || 0 });
            if (endTime && endTime >= 0) {
                youtubeOpts.playerVars = Object.assign(Object.assign({}, youtubeOpts.playerVars), { end: endTime || 0 });
            }
        }
    }
    else {
    ...
    const saveState = ({ isVideoPlayerStarted, currentTime = '0', duration = '0', isVideoCompleted, videoState, }) => {
    ...
    const iframeTag = (<YouTube videoId={videoId} containerClassName="react-youtube-container" opts={youtubeOpts} onPlay={handleVideoPlay} onEnd={handleVideoEnd} onPause={handleVideoPause}/>);
    const srcAsWebm = (finalSrc === null || finalSrc === void 0 ? void 0 : finalSrc.substring(0, finalSrc === null || finalSrc === void 0 ? void 0 : finalSrc.lastIndexOf('.'))) +
        `.webm#t=${startTime}${endTime > 0 ? `,${endTime}` : ''}`;
    const videoTag = (<video width="100%" height="100%" 
    /* className={cssClass} */
    autoPlay={autoPlay} controls={!_videoIsCompleted || enableReplay} onEnded={handleVideoEnd} onPlay={handleVideoPlay} onPause={handleVideoPause}>
      <source src={finalSrc}/>
    ...
    return ready ? (<div data-janus-type={tagName} style={{ width: '100%', height: '100%' }}>
      <style>
        {`
