...
import { makeChoice, makeHint, makePart, makeUndoable, } from 'components/activities/types';
...
export const MultiInputActions = {
    editStemAndPreviewText(content, editor, operations) {
        return (model, post) => {
            const removedInputRefs = elementsRemoved(operations, 'input_ref');
            // Handle error condition - removing an extra input ref that is not present in the model
            if (removedInputRefs.length > 0 &&
            ...
            if (operations.find((op) => op.type === 'insert_node' &&
            ...
            const inputRefIds = elementsOfType(editor, 'input_ref').map(({ id }) => id);
            MultiInputActions.reorderInputs(inputRefIds)(model);
            MultiInputActions.reorderPartsByInputs()(model);
        };
    },
    addMissingParts(operations) {
        return (model) => {
            elementsAdded(operations, 'input_ref').forEach((inputRef) => MultiInputActions.addPart(inputRef.id)(model));
        };
    },
    removeExtraParts(operations) {
        return (model, post) => {
            const removedInputRefs = elementsRemoved(operations, 'input_ref');
            const clonedStem = clone(model.stem);
            const clonedPreviewText = clone(model.authoring.previewText);
            removedInputRefs.forEach((inputRef) => MultiInputActions.removePart(inputRef.id, clonedStem, clonedPreviewText)(model, post));
        };
    },
    addChoice(inputId, choice) {
        return (model) => {
            const input = model.inputs.find((input) => input.id === inputId);
            if (!input || input.inputType !== 'dropdown')
                return;
            Choices.addOne(choice)(model);
            input.choiceIds.push(choice.id);
        };
    },
    reorderChoices(inputId, dropdownChoices) {
        return (model) => {
            const input = model.inputs.find((input) => input.id === inputId);
            if (!input || input.inputType !== 'dropdown')
                return;
            model.choices = model.choices.filter((choice) => !dropdownChoices.map(({ id }) => id).includes(choice.id));
            model.choices.push(...dropdownChoices);
        };
    },
    reorderPartsByInputs() {
        return (model) => {
            const { getOne, setAll } = List('$..parts');
    ...
    removeChoice(inputId, choiceId) {
        return (model, post) => {
            ...
            post(makeUndoable('Removed a choice', [
    ...
    setInputType(id, type) {
        return (model) => {
            ...
            if (type === 'dropdown') {
                ...
                input.choiceIds = choices.map(({ id }) => id);
            }
            part.responses = {
    ...
    removeTargetedMappingsForPart(part) {
        return (model) => {
            model.authoring.targeted = model.authoring.targeted.filter(([, responseId]) => !part.responses.find(({ id }) => id === responseId));
        };
    },
    removeChoicesForInput(dropdown) {
        return (model) => {
            model.choices = model.choices.filter((c) => !dropdown.choiceIds.includes(c.id));
        };
    },
    removePart(inputId, stem, previewText) {
        return (model, post) => {
