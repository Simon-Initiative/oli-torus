...
import { activityDeliverySlice, initializeState, isEvaluated, resetAction, } from 'data/activities/DeliveryState';
...
export const MultiInputComponent = () => {
    const { state: activityState, onSaveActivity, onResetActivity, model, sectionSlug, } = useDeliveryElementContext();
    const uiState = useSelector((state) => state);
    const [hintsShown, setHintsShown] = React.useState([]);
    const dispatch = useDispatch();
    useEffect(() => {
        dispatch(initializeState(activityState, safelySelectInputs(activityState).caseOf({
            just: (inputs) => inputs,
            nothing: () => model.inputs.reduce((acc, input) => {
                acc[input.partId] = [''];
                return acc;
            }, {}),
        })));
    ...
    const toggleHints = (id) => {
        ...
        setHintsShown((hintsShown) => hintsShown.includes(input.partId)
            ? hintsShown.filter((id) => id !== input.partId)
            : hintsShown.concat(input.partId));
    };
    const inputs = new Map(model.inputs.map((input) => {
        var _a;
        return [
            input.id,
            {
                input: input.inputType === 'dropdown'
                    ? {
                        id: input.id,
                ...
                value: (((_a = uiState.partState[input.partId]) === null || _a === void 0 ? void 0 : _a.studentInput) || [''])[0],
    ...
    return (<div className="activity mc-activity">
      <div className="activity-content">
        <StemDelivery className="form-inline" stem={model.stem} context={writerContext}/>
        <GradedPointsConnected />
        <ResetButtonConnected onReset={() => dispatch(resetAction(onResetActivity, model.inputs.reduce((acc, input) => {
            acc[input.partId] = [''];
            return acc;
        }, {})))}/>
        <SubmitButtonConnected disabled={false}/>
        {hintsShown.map((partId) => (<HintsDeliveryConnected key={partId} partId={partId} shouldShow={hintsShown.includes(partId)}/>))}
        <EvaluationConnected />
      </div>
    </div>);
};
// Defines the web component, a simple wrapper over our React component above
export class MultiInputDelivery extends DeliveryElement {
...
//# sourceMappingURL=MultiInputDelivery.jsx.map