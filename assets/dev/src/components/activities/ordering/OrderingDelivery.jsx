...
import { activityDeliverySlice, initializeState, resetAction, } from 'data/activities/DeliveryState';
...
export const OrderingComponent = () => {
    var _a;
    const { model, state: activityState, onResetActivity, onSaveActivity, } = useDeliveryElementContext();
    const uiState = useSelector((state) => state);
    const dispatch = useDispatch();
    const onSelectionChange = (studentInput) => {
    ...
    useEffect(() => {
        dispatch(initializeState(activityState, initialPartInputs(activityState, { [DEFAULT_PART_ID]: model.choices.map((c) => c.id) })));
    ...
    return (<div className="activity ordering-activity">
      <div className="activity-content">
        <StemDeliveryConnected />
        <GradedPointsConnected />
        <ResponseChoices choices={Maybe.maybe((_a = uiState.partState[DEFAULT_PART_ID]) === null || _a === void 0 ? void 0 : _a.studentInput)
            .valueOr([])
            .map((id) => Choices.getOne(model, id))} setChoices={(choices) => onSelectionChange(choices.map((c) => c.id))}/>
...
export class OrderingDelivery extends DeliveryElement {
...
//# sourceMappingURL=OrderingDelivery.jsx.map