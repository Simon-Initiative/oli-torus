...
import { getChoiceIds, getCorrectChoiceIds, getCorrectResponse, getResponseBy, getResponseId, getResponses, } from 'data/activities/model/responses';
...
export class Actions {
    ...
    static setCorrectChoices(choices) {
        return (model) => {
            model.authoring.correct[0] = choices.map((c) => c.id);
            updateResponseRules(model);
        };
    }
    static removeChoiceAndUpdateRules(id) {
        return (model, post) => {
    ...
    static editTargetedFeedbackChoices(responseId, choiceIds) {
        return (model) => {
...
const updateResponseRules = (model) => {
    ...
    model.authoring.targeted.forEach((assoc) => {
        getResponseBy(model, (r) => r.id === getResponseId(assoc)).rule = matchInOrderRule(getChoiceIds(assoc));
    });
};
