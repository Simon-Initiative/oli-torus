...
export const Ordering = () => {
    ...
    return (<TabbedNavigation.Tabs>
      <TabbedNavigation.Tab label="Question">
        <Stem />
        <ChoicesAuthoring icon={(choice, index) => <span className="mr-1">{index + 1}.</span>} choices={model.choices} addOne={() => dispatch(Actions.addChoice(ActivityTypes.makeChoice('')))} setAll={(choices) => dispatch(Choices.setAll(choices))} onEdit={(id, content) => dispatch(Choices.setContent(id, content))} onRemove={(id) => dispatch(Actions.removeChoiceAndUpdateRules(id))}/>
      </TabbedNavigation.Tab>

      <TabbedNavigation.Tab label="Answer Key">
        <ResponseChoices choices={getCorrectChoiceIds(model).map((id) => Choices.getOne(model, id))} setChoices={(choices) => dispatch(Actions.setCorrectChoices(choices))}/>
...
export class OrderingAuthoring extends AuthoringElement {
...
//# sourceMappingURL=OrderingAuthoring.jsx.map