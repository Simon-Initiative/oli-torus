...
const Adaptive = (props) => {
    const { content: { custom: config, partsLayout }, } = props.model;
    ...
    const [adaptivityDomain, setAdaptivityDomain] = useState('stage');
    ...
    useEffect(() => {
        ...
        sharedInitMap.set(props.model.id, parts.reduce((collect, part) => {
    ...
    const partInit = useCallback((partId) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (parts.every((part) => partsInitStatus[part.id] === true)) {
            if (props.onReady) {
                const readyResults = yield props.onReady(currentAttemptState.attemptGuid);
                ...
                partsInitDeferred.resolve({
                    ...
                    context: Object.assign(Object.assign({}, readyResults.context), { host: props.mountPoint, domain: domain || adaptivityDomain }),
                    env,
                });
            }
            else {
    ...
    const handlePartInit = (payload) => __awaiter(void 0, void 0, void 0, function* () {
        /* console.log('onPartInit', payload); */
        // a part should send initial state values
        if (payload.responses.length) {
            const saveResults = yield handlePartSave(payload);
        }
        const { snapshot, context, env } = yield partInit(payload.id.toString());
        // TODO: something with save result? check for errors?
        return { snapshot, context, env };
    });
    const handlePartReady = (payload) => __awaiter(void 0, void 0, void 0, function* () {
        /* console.log('onPartReady', { payload }); */
        return true;
    });
    const handlePartResize = (payload) => __awaiter(void 0, void 0, void 0, function* () {
        // no need to do anything for now.
        /*  console.log('handlePartResize called'); */
        return true;
    });
    const handleSetData = (payload) => __awaiter(void 0, void 0, void 0, function* () {
        const currentAttemptState = sharedAttemptStateMap.get(props.model.id);
        // part attempt guid should be located in currentAttemptState.parts matched to id
        const partAttempt = currentAttemptState.parts.find((p) => p.partId === payload.id);
        if (!partAttempt) {
        ...
        if (props.onWriteUserState) {
            yield props.onWriteUserState(currentAttemptState.attemptGuid, partAttempt === null || partAttempt === void 0 ? void 0 : partAttempt.attemptGuid, payload);
        }
    });
    const handleGetData = (payload) => __awaiter(void 0, void 0, void 0, function* () {
        const currentAttemptState = sharedAttemptStateMap.get(props.model.id);
        // part attempt guid should be located in currentAttemptState.parts matched to id
        const partAttempt = currentAttemptState.parts.find((p) => p.partId === payload.id);
        if (!partAttempt) {
        ...
        if (props.onReadUserState) {
            return yield props.onReadUserState(currentAttemptState.attemptGuid, partAttempt === null || partAttempt === void 0 ? void 0 : partAttempt.attemptGuid, payload);
        }
    });
    const handlePartSave = ({ id, responses }) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const response = {
            input: responses.map((pr) => (Object.assign(Object.assign({}, pr), { path: `${id}.${pr.key}` }))),
        };
        const result = yield props.onSavePart(currentAttemptState.attemptGuid, partAttempt === null || partAttempt === void 0 ? void 0 : partAttempt.attemptGuid, response);
        // BS: this is the result from the layout pushed down, need to push down to part here?
        return result;
    });
    const handlePartSubmit = ({ id, responses }) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const result = yield props.onSubmitPart(currentAttemptState.attemptGuid, partAttempt === null || partAttempt === void 0 ? void 0 : partAttempt.attemptGuid, response);
        // BS: this is the result from the layout pushed down, need to push down to part here?
        return result;
    });
    return init ? (<NotificationContext.Provider value={pusher}>
      <PartsLayoutRenderer parts={parts} onPartInit={handlePartInit} onPartReady={handlePartReady} onPartSave={handlePartSave} onPartSubmit={handlePartSubmit} onPartResize={handlePartResize} onPartSetData={handleSetData} onPartGetData={handleGetData}/>
    </NotificationContext.Provider>) : null;
};
// Defines the web component, a simple wrapper over our React component above
export class AdaptiveDelivery extends DeliveryElement {
...
//# sourceMappingURL=AdaptiveDelivery.jsx.map