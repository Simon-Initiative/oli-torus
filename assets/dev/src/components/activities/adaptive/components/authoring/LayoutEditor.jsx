...
const LayoutEditor = (props) => {
    ...
    useEffect(() => {
        const x = (selectedPart === null || selectedPart === void 0 ? void 0 : selectedPart.custom.x) || 0;
        const y = ((selectedPart === null || selectedPart === void 0 ? void 0 : selectedPart.custom.y) || 0) + toolBarTopOffset;
    ...
    useEffect(() => {
        if (selectedPartId) {
            ...
            if (part) {
                let capabilities = Object.assign({}, defaultCapabilities);
                ...
                if (PartClass) {
                    ...
                    const instance = new PartClass(); // TODO: extend HTMLElement?
                    if (instance.getCapabilities) {
                        capabilities = Object.assign(Object.assign({}, capabilities), instance.getCapabilities());
                    }
                    const partWithCapabilities = Object.assign(Object.assign({}, part), { capabilities });
    ...
    useEffect(() => {
        const handleHostClick = (e) => {
            const path = e.path;
            const pathIds = (path === null || path === void 0 ? void 0 : path.map((node) => node.getAttribute && node.getAttribute('id'))) || [];
    ...
    const handlePartClick = useCallback((e, payload) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const handlePartDrag = useCallback((payload) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const part = partsClone.find((p) => p.id === payload.partId);
        if (part) {
    ...
    const handlePartConfigure = useCallback((partId, configure, context) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const handleToolbarPartConfigure = (partId, configure) => {
        pusher.emit(NotificationType.CONFIGURE.toString(), { partId, configure });
    };
    const handlePartDelete = useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const handleCopyComponent = useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const handlePartMoveForward = useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
        /* console.log('AUTHOR PART MOVE FWD', { selectedPart }); */
        const partsClone = clone(parts);
        const part = partsClone.find((p) => p.id === selectedPart.id);
        part.custom.z = part.custom.z + 1;
        props.onChange(partsClone);
        // optimistically update local state
        setParts(partsClone);
    }), [selectedPart, parts]);
    const handlePartMoveBack = useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const handlePartCancelConfigure = useCallback(({ id }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const handlePartSaveConfigure = useCallback(({ id, snapshot }) => __awaiter(void 0, void 0, void 0, function* () {
        const partsClone = clone(parts);
        const part = partsClone.find((p) => p.id === id);
        if (part) {
    ...
    useEffect(() => {
        ...
        parts.forEach((part) => {
            ...
            if (partElement) {
                partElement.setAttribute('style', `transform: translate(${part.custom.x || 0}px, ${part.custom.y || 0}px);`);
            }
        });
    }, [parts]);
    useEffect(() => {
    ...
    return parts && parts.length ? (<NotificationContext.Provider value={pusher}>
      <div ref={containerRef} className="activity-content">
        ...
        <div className="part-config-container" style={{ display: configurePartId.trim() ? 'block' : 'none' }} onClick={handlePortalBgClick}>
          <div id={fallbackPortalId} className="part-config-container-inner"></div>
        </div>
        <div id={`active-selection-toolbar-${props.id}`} className="active-selection-toolbar" style={{
            display: selectedPart && !isDragging ? 'block' : 'none',
            top: toolbarPosition.y,
            left: toolbarPosition.x,
        }}>
          {selectedPart && selectedPart.capabilities.configure && (<button title="Edit" onClick={() => handleToolbarPartConfigure(selectedPart.id, true)}>
              <i className="las la-edit"></i>
            </button>)}
          {selectedPart && selectedPart.capabilities.copy && (<button title="Copy" onClick={handleCopyComponent}>
              <i className="las la-copy"></i>
            </button>)}
          {/* <button title="Configure" onClick={handlePartConfigure}>
          <i className="las la-cog"></i>
        </button> */}
          {selectedPart && selectedPart.capabilities.move && (<button title="Move Forward" onClick={handlePartMoveForward}>
              <i className="las la-plus"></i>
            </button>)}
          {selectedPart && selectedPart.capabilities.move && (<button title="Move Back" onClick={handlePartMoveBack}>
              <i className="las la-minus"></i>
            </button>)}
          {selectedPart && selectedPart.capabilities.delete && (<button title="Delete" onClick={() => setShowConfirmDelete(true)}>
              <i className="las la-trash"></i>
            </button>)}
          <ConfirmDelete show={showConfirmDelete} elementType="Component" elementName={selectedPart === null || selectedPart === void 0 ? void 0 : selectedPart.id} deleteHandler={DeleteComponentHandler} cancelHandler={() => {
            setShowConfirmDelete(false);
        }}/>
        </div>
        {parts.map((part) => {
            ...
            const disableDrag = selectedPartId !== part.id ||
                part.id === configurePartId ||
                (selectedPart && !selectedPart.capabilities.move);
            return (<Draggable key={part.id} grid={[5, 5]} defaultPosition={{ x: part.custom.x || 0, y: part.custom.y || 0 }} disabled={disableDrag} onStart={() => {
                    setIsDragging(true);
                }} onStop={(_, dragData) => {
                    setIsDragging(false);
                    handlePartDrag({ partId: part.id, dragData });
                }}>
              <PartComponent {...partProps} className={selectedPartId === part.id ? 'selected' : ''} onClick={(event) => handlePartClick(event, { id: part.id })} onConfigure={({ configure, context }) => handlePartConfigure(part.id, configure, context)} onSaveConfigure={handlePartSaveConfigure} onCancelConfigure={handlePartCancelConfigure}/>
            </Draggable>);
        })}
      </div>
...
//# sourceMappingURL=LayoutEditor.jsx.map