...
import { initializeState, setSelection, activityDeliverySlice, isEvaluated, resetAction, } from 'data/activities/DeliveryState';
...
export const MultipleChoiceComponent = () => {
    const { state: activityState, onSaveActivity, onResetActivity, } = useDeliveryElementContext();
    ...
    return (<div className="activity mc-activity">
      <div className="activity-content">
        ...
        <ChoicesDeliveryConnected partId={DEFAULT_PART_ID} unselectedIcon={<Radio.Unchecked disabled={isEvaluated(uiState)}/>} selectedIcon={!isEvaluated(uiState) ? (<Radio.Checked />) : isCorrect(uiState.attemptState) ? (<Radio.Correct />) : (<Radio.Incorrect />)} onSelect={(id) => dispatch(setSelection(DEFAULT_PART_ID, id, onSaveActivity, 'single'))}/>
...
//# sourceMappingURL=MultipleChoiceDelivery.jsx.map