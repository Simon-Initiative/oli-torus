...
export const Item = (props) => {
    ...
    return (<div className="my-3" key={feedback.id}>
      <Description>
        {score === 1 ? <Checkmark /> : <Cross />}
        Feedback for {score === 1 ? 'Correct' : 'Incorrect'} Answer:
      </Description>
      <RichTextEditor projectSlug={props.projectSlug} editMode={editMode} value={feedback.content} onRequestMedia={props.onRequestMedia} onEdit={(content) => onEditResponse(score, content)}/>
    </div>);
};
export const Feedback = (props) => {
    ...
    return (<div className="my-5">
      ...
      {model.feedback.map((f, index) => (<Item key={index} {...props} feedback={f} score={index}/>))}
    </div>);
};
