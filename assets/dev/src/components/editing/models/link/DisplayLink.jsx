...
import { isInternalLink, toInternalLink, translateDeliveryToAuthoring, } from './utils';
export const DisplayLink = (props) => {
    const { href, pages, setPages, selectedPage, setSelectedPage, setEditLink, commandContext } = props;
    const onCopy = (href) => {
    ...
    const onVisit = (href) => {
        window.open(isInternalLink(href) ? translateDeliveryToAuthoring(href, commandContext.projectSlug) : href, '_blank');
    };
    const fetchPages = () => {
    ...
    return (<div className="hovering-toolbar">
      <div className="btn-group btn-group-sm" role="group">
        <FormattingToolbar commandDescs={pages.type === 'success' ? linkCommands : loadingCommand} commandContext={commandContext}/>
      </div>
    </div>);
};
//# sourceMappingURL=DisplayLink.jsx.map