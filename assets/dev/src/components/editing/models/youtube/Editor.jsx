...
export const YouTubeEditor = (props) => {
    ...
    const fullSrc = 'https://www.youtube.com/embed/' + (src === '' ? CUTE_OTTERS : src) + '?' + parameters;
    ...
    // events.
    return (<div {...attributes} style={{ userSelect: 'none' }} className={'youtube-editor ' + displayModelToClassName(model.display)}>
      <div contentEditable={false} onClick={(_e) => Transforms.select(editor, ReactEditor.findPath(editor, model))} className="embed-responsive embed-responsive-16by9 img-thumbnail" style={borderStyle}>
        <iframe className="embed-responsive-item" src={fullSrc} allowFullScreen></iframe>
      </div>
      <div contentEditable={false}>
        <Settings.Input editMode={editMode} value={model.caption} onChange={setCaption} editor={editor} model={model} placeholder="Set a caption for this YouTube video"/>
      </div>
