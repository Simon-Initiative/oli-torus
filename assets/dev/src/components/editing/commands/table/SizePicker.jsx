...
const initialSize = { rows: 1, columns: 1 };
const minRows = 4;
const minCols = 4;
const maxRows = 15;
const maxCols = 15;
...
export const SizePicker = (props) => {
    ...
    const mapRow = (row) => {
        const cells = cols.map((col) => (<div key={'col' + col} style={cellContainerStyle} onMouseEnter={(e) => setSize({ rows: row, columns: col })} onMouseDown={(e) => {
    ...
    return (<div className="table-size-picker" style={gridStyle}>
      {rows.map(mapRow)}
...
//# sourceMappingURL=SizePicker.jsx.map