...
const offsetByResizeHandleSize = (styles) => Object.assign(styles, {
...
const positions = ({ left, top, width, height }) => (position) => {
    switch (position) {
        case 'border':
...
export const resizeHandleStyles = (boundingRect) => (position) => Object.assign(position === 'border'
    ? positions(boundingRect)(position)
    : offsetByResizeHandleSize(positions(boundingRect)(position)), cursor(position));
export const clientBoundingRect = (element) => {
...
export const offsetBoundingRect = (element) => {
...
const proportionalWidthAndHeight = ({ width: initialWidth, height: initialHeight }, boundaryWidth, boundaryHeight) => {
...
export const boundingRectFromMousePosition = (initialClientBoundingRect, initialOffsetBoundingRect, mousePosition, dragHandle) => constrainRect(boundingRectHelper(initialClientBoundingRect, initialOffsetBoundingRect, mousePosition, dragHandle), initialOffsetBoundingRect);
const constrainRect = ({ top, left, width, height }, { top: initialTop, left: initialLeft, width: initialWidth, height: initialHeight }) => {
    const MIN_SIZE = 10;
    const atLeast = (a, min) => (a < min ? min : a);
    const atMost = (a, max) => (a > max ? max : a);
    return {
...
const boundingRectHelper = (initialClientBoundingRect, initialOffsetBoundingRect, { x, y }, dragHandle) => {
    const { top: offsetTop, left: offsetLeft, width: offsetWidth, height: offsetHeight, } = initialOffsetBoundingRect;
    const { top: clientTop, left: clientLeft, width: clientWidth, height: clientHeight, } = initialClientBoundingRect;
    let dw = 0, dh = 0, proportional;
    switch (true) {
        case dragHandle === 'nw':
            dw = x - clientLeft;
            dh = y - clientTop;
            proportional = proportionalWidthAndHeight(initialOffsetBoundingRect, offsetWidth - dw, offsetHeight - dh);
            return {
