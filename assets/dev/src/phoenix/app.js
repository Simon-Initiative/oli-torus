...
window.initActivityBridge = initActivityBridge;
window.initPreviewActivityBridge = initPreviewActivityBridge;
// Expose React/Redux APIs to server-side rendered templates
function mount(Component, element, context = {}) {
...
window.OLI = {
    ...
    retrieveCookies,
    onReady,
    CreateAccountPopup: (node, props) => mount(CreateAccountPopup, node, props),
};
...
$(() => {
    ...
    $('[data-toggle="popover"]').on('focus', (e) => {
        $('[data-toggle="popover"]:not(.popup__click)').popover('hide');
    ...
    $('body').on('mousedown', (e) => {
        const isPopover = (e) => $(e.target).data('toggle') === 'popover';
        const isClickable = (e) => $(e.target).hasClass('popup__click');
        const isPopupContent = (e) => $(e.target).parents('.popup__content').length > 0;
        const isFocused = (e) => document.activeElement && $(document.activeElement).is($(e.target));
        ...
        if (isPopover(e) && isClickable(e) && isFocused(e)) {
            return $(e.target).popover('toggle');
        }
    });
