...
export const looksLikeJson = (str) => {
    const emptyJsonObj = '{}';
    const jsonStart1 = '{"';
    const jsonStart2 = '{ "';
    const jsonEnd = '}';
...
export const getExpressionStringForValue = (v, env = defaultGlobalEnv) => {
    let val = v.value;
    ...
    if (typeof val === 'string') {
        ...
        try {
            ...
            canEval = (test === null || test === void 0 ? void 0 : test.result) !== undefined && !test.result.message;
    ...
    if (isValueVar) {
        ...
        if (isCSSString === null || isCSSString === void 0 ? void 0 : isCSSString.length) {
        ...
        if (!actuallyAString) {
            try {
                ...
                if ((testResult === null || testResult === void 0 ? void 0 : testResult.result) !== null) {
                    //expression {stage.foo} + {stage.bar} was failling if we set actuallyAString= true
                    actuallyAString = (expressions === null || expressions === void 0 ? void 0 : expressions.length) ? false : true;
                }
            }
            catch (e) {
    ...
    if (v.type === CapiVariableTypes.STRING ||
...
export const evalScript = (script, env) => {
    var _a;
    ...
    if (((_a = jsResult === null || jsResult === void 0 ? void 0 : jsResult.message) === null || _a === void 0 ? void 0 : _a.indexOf('is a bound reference, cannot assign')) > 0) {
        return { env: globalEnv, result: null };
    }
    return { env: globalEnv, result: jsResult };
};
export const evalAssignScript = (state, env) => {
    const globalEnv = env || new Environment();
...
export const getAssignStatements = (state, env = defaultGlobalEnv) => {
    const vars = Object.keys(state).map((key) => {
        ...
        if (val && val.constructor && val.constructor === Object) {
            if (val.key || val.path) {
                ...
                writeVal = Object.assign(Object.assign({}, val), { key: val.path ? val.path : val.key });
            }
            else {
    ...
    const letStatements = vars.map((v) => `let {${v.key.trim()}} = ${getExpressionStringForValue(v, env)};`);
    return letStatements;
};
export const getAssignScript = (state, env = defaultGlobalEnv) => {
    const letStatements = getAssignStatements(state, env);
    return letStatements.join('');
};
export const getEnvState = (env) => {
    // filter out functions, TODO: should do this perhaps in the lib instead?
    const dump = env.toObj();
    const filtered = Object.keys(dump).reduce((collect, key) => {
...
export const getValues = (identifiers, env) => {
    const jIds = identifiers.map((id) => `{${id}}`);
    const script = `[${jIds.join(',')}]`;
    const { result } = evalScript(script, env);
    return result;
};
export const getValue = (identifier, env) => {
    const script = `{${identifier}}`;
    const { result } = evalScript(script, env);
    return result;
};
export const applyState = (operation, env = defaultGlobalEnv) => {
...
export const bulkApplyState = (operations, env = defaultGlobalEnv) => {
    // need to apply one at a time, TODO: break on error?
    return operations.map((op) => applyState(op, env));
};
export const removeStateValues = (env, keys) => {
    env.remove(keys);
};
export const getLocalizedStateSnapshot = (activityIds, env = defaultGlobalEnv) => {
    const snapshot = getEnvState(env);
    const finalState = Object.assign({}, snapshot);
...
export const extractExpressionFromText = (text) => {
...
export const extractAllExpressionsFromText = (text) => {
    const expressions = [];
