...
export const isAnyOfOperator = (factValue, value) => {
...
export const isEqual = (factValue, value) => {
...
export const notIsAnyOfOperator = (factValue, value) => !isAnyOfOperator(factValue, value);
export const isNaNOperator = (factValue, value) => parseBoolean(value) === (Number.parseFloat(factValue).toString() === 'NaN');
export const equalWithToleranceOperator = (factValue, value) => {
    ...
    const isInRange = modifiedFactValue >= valuesWithTolerance.minToleranceValue &&
...
export const getValueWithTolerance = (baseMinValue, baseMaxValue, tolerance) => {
    let newValue = {
    ...
    if (tolerance > 0) {
        ...
        const minToleranceValue = baseMinValue >= 0
            ? baseMinValue - calculateMinWithToleranceValue
            : baseMinValue + calculateMinWithToleranceValue;
        const maxToleranceValue = baseMinValue >= 0
...
export const notEqualWithToleranceOperator = (factValue, value) => !equalWithToleranceOperator(factValue, value);
export const notEqual = (factValue, value) => {
    const typeOfValue = typeof value;
    //the rules that check the actual numbers do NOT fire if the value is NaN or the text box isn't filled out.
    //so `is not 32.06` should not fire true if the number is a NaN.
    if (typeOfValue === 'number' && Number.isNaN(factValue)) {
