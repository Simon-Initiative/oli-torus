...
const Title = (props) => {
    ...
    return (<div className="pt-header">
      <button onClick={() => togglePanel()}>
        <svg fill={window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches
            ? '#ffffff'
            : '#000000'} height="24" viewBox="0 0 14 14" width="24" xmlns="http://www.w3.org/2000/svg">
...
// Reusable Icons
const ScreensIcon = () => (<svg className="dock__icon" fill={window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches
        ? '#ffffff'
        : '#000000'} height="24" viewBox="0 0 18 18" width="24" xmlns="http://www.w3.org/2000/svg">
...
const AdaptivityIcon = () => (<svg className="dock__icon" fill={window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches
        ? '#ffffff'
        : '#000000'} height="24" viewBox="0 0 18 18" width="24" xmlns="http://www.w3.org/2000/svg">
...
const InspectorIcon = () => (<svg className="dock__icon" fill={window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches
        ? '#ffffff'
        : '#000000'} height="24" viewBox="0 0 18 18" width="24" xmlns="http://www.w3.org/2000/svg">
...
const PreviewTools = ({ model }) => {
    var _a;
    const [opened, setOpened] = useState(false);
    const [view, setView] = useState('screens');
    const currentActivity = useSelector(selectCurrentActivity);
    const dispatch = useDispatch();
    const sequence = (_a = model[0].children) === null || _a === void 0 ? void 0 : _a.filter((child) => !child.custom.isLayer && !child.custom.isBank).map((s) => {
        return Object.assign({}, s.custom);
    });
    ...
    return (<div id="PreviewTools" className={`preview-tools${opened ? ' opened' : ''}`}>
      {opened && (<Title togglePanel={togglePanel} title={view.charAt(0).toUpperCase() + view.slice(1)}/>)}

      <div className="pt-body">
        {!opened && (<div className="action-picker">
            <button onClick={() => displayView('screens')} className="mb-2" style={{
                border: 'none',
                background: 'transparent',
            }}>
              <ScreensIcon />
            </button>
            <button onClick={() => displayView('adaptivity')} className="mb-2" style={{
                border: 'none',
                background: 'transparent',
            }}>
              <AdaptivityIcon />
            </button>
            <button onClick={() => displayView('inspector')} style={{
                border: 'none',
                background: 'transparent',
            }}>
              <InspectorIcon />
        ...
        {opened && view === 'screens' && (<ScreenSelector sequence={sequence} navigate={navigate} currentActivity={currentActivity}/>)}
...
//# sourceMappingURL=PreviewTools.jsx.map