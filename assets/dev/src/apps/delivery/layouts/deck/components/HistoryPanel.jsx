...
const HistoryPanel = ({ items, onMinimize, onRestart }) => {
    ...
    const itemClickHandler = (item) => {
        ...
        const nextHistoryActivityIndex = items.findIndex((historyItem) => historyItem.id === item.id);
    ...
    return (<>
      {isLegacyTheme ? (<>
          ...
          <nav className="history">
            {items.map((item, index) => (<div key={item.id} id={`qrID${item.id}`} className={getItemClasses(item)}>
                {items.length - index}. {item.name}
              </div>))}
          </nav>
        </>) : (<div id="theme-history-panel" className="theme-history" aria-role="alertdialog" aria-hidden="false" aria-label="Lesson history">
          <div className="theme-history__title">
            <span>Lesson History</span>
            <button onClick={onMinimize} className="theme-history__close-btn" aria-label="Minimize lesson history">
              <span>
          ...
          <nav className="theme-history__nav">
            {items.map((item) => (<button key={item.id} className={getItemClasses(item)} onClick={() => itemClickHandler(item)}>
...
//# sourceMappingURL=HistoryPanel.jsx.map