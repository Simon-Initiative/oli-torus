...
import { selectRightPanelActiveTab, setCopiedPart, setRightPanelActiveTab, } from '../../../authoring/store/app/slice';
...
import { updateSequenceItem, updateSequenceItemFromActivity, } from '../../store/groups/layouts/deck/actions/updateSequenceItemFromActivity';
...
import bankSchema, { bankUiSchema, transformBankModeltoSchema, transformBankSchematoModel, } from '../PropertyEditor/schemas/bank';
import bankPropsSchema, { BankPropsUiSchema, transformBankPropsModeltoSchema, transformBankPropsSchematoModel, } from '../PropertyEditor/schemas/bankScreen';
import lessonSchema, { lessonUiSchema, transformModelToSchema as transformLessonModel, transformSchemaToModel as transformLessonSchema, } from '../PropertyEditor/schemas/lesson';
import partSchema, { partUiSchema, transformModelToSchema as transformPartModelToSchema, transformSchemaToModel as transformPartSchemaToModel, } from '../PropertyEditor/schemas/part';
import screenSchema, { screenUiSchema, transformScreenModeltoSchema, transformScreenSchematoModel, } from '../PropertyEditor/schemas/screen';
export var RightPanelTabs;
(function (RightPanelTabs) {
    RightPanelTabs["LESSON"] = "lesson";
    RightPanelTabs["SCREEN"] = "screen";
    RightPanelTabs["COMPONENT"] = "component";
...
const RightMenu = () => {
    ...
    const [scrSchema, setScreenSchema] = useState();
    const [scrUiSchema, setScreenUiSchema] = useState();
    const [questionBankData, setBankData] = useState();
    const [questionBankSchema, setBankSchema] = useState();
    ...
    useEffect(() => {
        ...
        setScreenData((currentSequence === null || currentSequence === void 0 ? void 0 : currentSequence.custom.isBank)
            ? transformBankPropsModeltoSchema(currentActivity)
            : transformScreenModeltoSchema(currentActivity));
        setScreenSchema((currentSequence === null || currentSequence === void 0 ? void 0 : currentSequence.custom.isBank) ? bankPropsSchema : screenSchema);
        setScreenUiSchema((currentSequence === null || currentSequence === void 0 ? void 0 : currentSequence.custom.isBank) ? BankPropsUiSchema : screenUiSchema);
        setBankData(transformBankModeltoSchema(currentSequence));
        setBankSchema(bankSchema);
        const currentIds = currentActivityTree === null || currentActivityTree === void 0 ? void 0 : currentActivityTree.reduce((acc, activity) => acc.concat(activity.content.partsLayout.map((p) => p.id)), []);
    ...
    const handleCopyComponent = useCallback(() => {
        ...
        if (currentActivity && currentPartSelection) {
            const partDef = (_a = currentActivity.content) === null || _a === void 0 ? void 0 : _a.partsLayout.find((part) => part.id === currentPartSelection);
            if (!partDef) {
    ...
    const bankPropertyChangeHandler = useCallback((properties) => {
        if (currentSequence) {
    ...
    const debounceSaveBankSettings = useCallback(debounce((group, currentSequence, bankShowCount, bankEndTarget) => {
        if (currentSequence) {
            const cloneSequence = clone(currentSequence);
            cloneSequence.custom.bankShowCount = bankShowCount;
            cloneSequence.custom.bankEndTarget = bankEndTarget;
    ...
    }, 0, { maxWait: 10000, leading: false }), []);
    const screenPropertyChangeHandler = useCallback((properties) => {
        var _a;
        if (currentActivity) {
            const modelChanges = (currentSequence === null || currentSequence === void 0 ? void 0 : currentSequence.custom.isBank)
            ...
            const { title } = modelChanges, screenModelChanges = __rest(modelChanges, ["title"]);
            const screenChanges = Object.assign(Object.assign({}, (_a = currentActivity === null || currentActivity === void 0 ? void 0 : currentActivity.content) === null || _a === void 0 ? void 0 : _a.custom), screenModelChanges);
    ...
    const debounceSaveScreenSettings = useCallback(debounce((activity, currentActivity, group) => {
        /* console.log('SAVING ACTIVITY:', { activity }); */
        dispatch(saveActivity({ activity }));
        if (activity.title !== (currentActivity === null || currentActivity === void 0 ? void 0 : currentActivity.title)) {
            dispatch(updateSequenceItemFromActivity({ activity: activity, group: group }));
            dispatch(savePage());
        }
    }, 500, { maxWait: 10000, leading: false }), []);
    const debounceSavePage = useCallback(debounce((changes) => {
    ...
    const lessonPropertyChangeHandler = useCallback((properties) => {
        ...
        const lessonChanges = Object.assign(Object.assign(Object.assign({}, currentLesson), modelChanges), { custom: Object.assign(Object.assign({}, currentLesson.custom), modelChanges.custom) });
        //need to remove the allowNavigation property
        //making sure the enableHistory is present before removing that.
        if (lessonChanges.custom.enableHistory !== undefined &&
    ...
    const debouncePartPropertyChanges = useCallback(debounce((properties, partInstance, origActivity, origId) => {
        ...
        if (partInstance && partInstance.transformSchemaToModel) {
            modelChanges.custom = Object.assign(Object.assign({}, modelChanges.custom), partInstance.transformSchemaToModel(modelChanges.custom));
        }
        /* console.log('COMPONENT PROP CHANGED', { properties, modelChanges }); */
        dispatch(updatePart({ activityId: origActivity.id, partId: origId, changes: modelChanges }));
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < currentActivityTree.length; i++) {
            ...
            partDef = (_a = activity.content) === null || _a === void 0 ? void 0 : _a.partsLayout.find((part) => part.id === currentPartSelection);
            if (partDef) {
                break;
            }
        }
        ...
        if (partDef) {
            ...
            if (PartClass) {
                ...
                if (instance.transformModelToSchema) {
                    ...
                    data.custom = Object.assign(Object.assign({}, data.custom), instance.transformModelToSchema(partDef.custom));
                }
                ...
                if (instance.getSchema) {
                    ...
                    const newSchema = Object.assign(Object.assign({}, partSchema), { properties: Object.assign(Object.assign({}, partSchema.properties), { custom: { type: 'object', properties: Object.assign({}, customPartSchema) } }) });
                ...
                if (instance.getUiSchema) {
                    ...
                    const newUiSchema = Object.assign(Object.assign({}, partUiSchema), { custom: Object.assign({ 'ui:ObjectFieldTemplate': AccordionTemplate, 'ui:title': 'Custom' }, customPartUiSchema) });
    ...
    const componentPropertyChangeHandler = useCallback((properties) => {
        debouncePartPropertyChanges(properties, currentPartInstance, currentActivity, currentPartSelection);
    }, [currentActivity, currentPartInstance, currentPartSelection]);
    const handleEditComponentJson = (newJson) => {
        var _a;
        const cloneActivity = clone(currentActivity);
        const ogPart = (_a = cloneActivity.content) === null || _a === void 0 ? void 0 : _a.partsLayout.find((part) => part.id === currentPartSelection);
        if (!ogPart) {
    ...
    const handleDeleteComponent = useCallback(() => {
        ...
        const cloneActivity = clone(currentActivity);
        cloneActivity.authoring.parts = cloneActivity.authoring.parts.filter((part) => part.id !== currentPartSelection);
        cloneActivity.content.partsLayout = cloneActivity.content.partsLayout.filter((part) => part.id !== currentPartSelection);
        dispatch(saveActivity({ activity: cloneActivity }));
        dispatch(setCurrentSelection({ selection: '' }));
        dispatch(setRightPanelActiveTab({ rightPanelActiveTab: RightPanelTabs.SCREEN }));
    }, [currentPartSelection, currentActivity]);
    return (<Tabs className="aa-panel-section-title-bar aa-panel-tabs" activeKey={selectedTab} onSelect={handleSelectTab}>
      <Tab eventKey={RightPanelTabs.LESSON} title="Lesson">
        <div className="lesson-tab overflow-hidden">
          <PropertyEditor schema={lessonSchema} uiSchema={lessonUiSchema} value={lessonData} onChangeHandler={lessonPropertyChangeHandler}/>
        </div>
      </Tab>
      <Tab eventKey={RightPanelTabs.SCREEN} title="Screen">
        {currentActivity && currentSequence && (currentSequence === null || currentSequence === void 0 ? void 0 : currentSequence.custom.isBank) ? (<div className="bank-tab p-3">
            <PropertyEditor key={currentActivity.id} schema={questionBankSchema} uiSchema={bankUiSchema} value={questionBankData} onChangeHandler={bankPropertyChangeHandler}/>
          </div>) : null}
        <div className="screen-tab p-3 overflow-hidden">
          {currentActivity && scrData ? (<PropertyEditor key={currentActivity.id} schema={scrSchema} uiSchema={scrUiSchema} value={scrData} onChangeHandler={screenPropertyChangeHandler}/>) : null}
        </div>
      </Tab>
      <Tab eventKey={RightPanelTabs.COMPONENT} title="Component" disabled={!currentComponent}>
        {currentComponent && currentComponentData && (<div className="component-tab p-3 overflow-hidden">
            <ButtonToolbar aria-label="Component Tools">
              <ButtonGroup className="me-2" aria-label="First group">
                ...
                <CompJsonEditor onChange={handleEditComponentJson} jsonValue={currentComponentData} existingPartIds={existingIds}/>
                <Button variant="danger" onClick={() => setShowConfirmDelete(true)}>
                  <i className="fas fa-trash mr-2"/>
                </Button>
                <ConfirmDelete show={showConfirmDelete} elementType="Component" elementName={currentComponentData === null || currentComponentData === void 0 ? void 0 : currentComponentData.id} deleteHandler={DeleteComponentHandler} cancelHandler={() => {
                setShowConfirmDelete(false);
            }}/>
              </ButtonGroup>
            </ButtonToolbar>
            <PropertyEditor key={currentComponentData.id} schema={componentSchema} uiSchema={componentUiSchema} value={currentComponentData} onChangeHandler={componentPropertyChangeHandler}/>
          </div>)}
      </Tab>
    </Tabs>);
};
export default RightMenu;
//# sourceMappingURL=RightMenu.jsx.map