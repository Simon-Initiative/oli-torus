...
export const transformScreenModeltoSchema = (activity) => {
    var _a;
    if (activity) {
        ...
        return Object.assign(Object.assign({}, data), { title: (activity === null || activity === void 0 ? void 0 : activity.title) || '', Size: { width: data.width, height: data.height }, checkButton: { showCheckBtn: data.showCheckBtn, checkButtonLabel: data.checkButtonLabel }, max: { maxAttempt: data.maxAttempt, maxScore: data.maxScore }, palette: data.palette.useHtmlProps ? data.palette : schemaPalette });
    }
};
export const transformScreenSchematoModel = (schema) => {
    return {
        title: schema.title,
