...
import { findInSequence, getHierarchy, } from 'apps/delivery/store/features/groups/actions/sequence';
import { selectSequence } from 'apps/delivery/store/features/groups/selectors/deck';
import React, { Fragment, useEffect, useState } from 'react';
import { useSelector } from 'react-redux';
import { SequenceDropdown } from './SequenceDropdown';
const ScreenDropdownTemplate = (props) => {
    ...
    const onChangeHandler = (item, e, isNext) => {
        const itemId = isNext ? 'next' : item === null || item === void 0 ? void 0 : item.custom.sequenceId;
        onChange(itemId);
    };
    return (<Fragment>
      {label && <span className="form-label">{label}</span>}
      <div className={`dropdown ${dropDownCSSClass || 'screenDropdown'}`}>
        <button className={`${buttonCSSClass} form-control dropdown-toggle d-flex justify-content-between`} type="button" id={id} data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
          {buttonLabel}
          <i className="fas fa-caret-down my-auto"/>
        </button>
        <div className="dropdown-menu" aria-labelledby={id}>
          <SequenceDropdown items={hierarchy} onChange={onChangeHandler} value={props.value} showNextBtn={true}/>
        </div>
      </div>
    </Fragment>);
};
export default ScreenDropdownTemplate;
//# sourceMappingURL=ScreenDropdownTemplate.jsx.map