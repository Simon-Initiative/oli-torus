...
const ComponentSearchContextMenu = () => {
    ...
    const handlePartClick = useCallback((part) => {
        const [currentActivity] = (currentActivityTree === null || currentActivityTree === void 0 ? void 0 : currentActivityTree.slice(-1)) || [];
        if (!currentActivity) {
            return;
        }
        if (currentActivity.content.partsLayout.find((p) => p.id === part.id)) {
            setShow(!show);
    ...
    return (paths && (<>
        ...
        <Overlay show={show} target={target} placement="bottom" container={document.getElementById('advanced-authoring')} containerPadding={20} rootClose={true} onHide={() => setShow(false)}>
          <Popover id="search-popover">
            ...
            <Popover.Content>
              <ListGroup className="aa-parts-list">
                {allParts.map((part) => (<ListGroup.Item active={part.id === currentPartSelection} action onClick={() => handlePartClick(part)} key={part.id} className="d-flex align-items-center">
...
//# sourceMappingURL=ComponentSearchContextMenu.jsx.map