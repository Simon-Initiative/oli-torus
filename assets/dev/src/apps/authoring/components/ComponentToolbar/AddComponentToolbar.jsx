import { selectCopiedPart, selectPartComponentTypes, selectPaths, setRightPanelActiveTab, } from 'apps/authoring/store/app/slice';
...
const AddComponentToolbar = () => {
    ...
    const addPartToCurrentScreen = (newPartData) => {
        if (currentActivityTree) {
            const [currentActivity] = currentActivityTree.slice(-1);
            dispatch(addPart({ activityId: currentActivity.id, newPartData }));
        }
    };
    const handleAddComponent = useCallback((partComponentType) => {
    ...
    const handlePartMenuButtonClick = (event) => {
        setShowPartsMenu(!showPartsMenu);
        setPartsMenuTarget(event.target);
    };
    const handlePartPasteClick = (event) => {
        const newPartData = {
    ...
    return (<Fragment>
      <div className="btn-group align-items-center" role="group">
        {availablePartComponents
            ...
            .map((part) => (<OverlayTrigger key={part.slug} placement="bottom" delay={{ show: 150, hide: 150 }} overlay={<Tooltip id="button-tooltip" style={{ fontSize: '12px' }}>
      ...
      <div className="btn-group pl-3 ml-3 border-left align-items-center" role="group">
        {copiedPart ? (<OverlayTrigger placement="bottom" delay={{ show: 150, hide: 150 }} overlay={<Tooltip id="button-tooltip" style={{ fontSize: '12px' }}>
                Paste Component
              </Tooltip>}>
            <span>
        ...
        <Overlay show={showPartsMenu} target={partsMenuTarget} placement="bottom" container={document.getElementById('advanced-authoring')} containerPadding={20} rootClose={true} onHide={() => setShowPartsMenu(false)}>
          <Popover id="moreComponents-popover">
            ...
            <Popover.Content>
              <ListGroup className="aa-parts-list">
                {availablePartComponents
            .filter((part) => !frequentlyUsed.includes(part.slug))
            .map((part) => (<ListGroup.Item action onClick={() => handleAddComponent(part.slug)} key={part.slug} className="d-flex align-items-center">
...
//# sourceMappingURL=AddComponentToolbar.jsx.map