...
const ActionMutateEditor = (props) => {
    ...
    return (<div className="aa-action aa-mutate d-flex mb-2 form-inline align-items-center flex-nowrap">
      <label className="sr-only" htmlFor={`action-mutate-target-${uuid}`}>
        target
      </label>
      <div className="input-group input-group-sm flex-grow-1">
        <div className="input-group-prepend" title="target">
          <VariablePicker targetRef={targetRef} typeRef={typeRef} placement={OverlayPlacements.TOP} context="mutate"/>
        </div>
        <input type="text" className="form-control form-control-sm mr-2 flex-grow-1" id={`action-mutate-target-${uuid}`} defaultValue={target} onBlur={(e) => handleTargetChange(e)} title={target} placeholder="Target" ref={targetRef}/>
      </div>
      <label className="sr-only" htmlFor={`action-mutate-type-${uuid}`}>
        type
      </label>
      <select className="custom-select mr-2 form-control form-control-sm" id={`action-mutate-type-${uuid}`} value={targetType} onChange={(e) => handleTargetTypeChange(e)} ref={typeRef}>
      ...
      <select className="custom-select mr-2 form-control form-control-sm" id={`action-mutate-operator-${uuid}`} value={operator} onChange={(e) => handleOperatorChange(e)}>
        {actionOperatorOptions
            ...
            .map((option) => (<option key={option.key} value={option.value}>
              {option.text}
            </option>))}
      </select>
      ...
      <div className="input-group input-group-sm">
        ...
        <input type="text" className="form-control form-control-sm" id={`action-mutate-value-${uuid}`} value={value} onChange={(e) => handleValueChange(e)} title={value} placeholder="Value"/>
      </div>
      ...
      {showConfirmDelete && (<ConfirmDelete show={showConfirmDelete} elementType="Action" elementName="this mutate action" deleteHandler={() => {
                onDelete(action);
...
//# sourceMappingURL=ActionMutateEditor.jsx.map