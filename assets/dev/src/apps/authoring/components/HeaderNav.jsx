...
import { selectPaths, selectProjectSlug, selectReadOnly, selectRevisionSlug, setShowDiagnosticsWindow, } from '../store/app/slice';
...
const HeaderNav = (props) => {
    ...
    return (paths && (<nav className={`aa-header-nav top-panel overflow-hidden${isVisible ? ' open' : ''} d-flex aa-panel-section-title-bar`} style={{
            alignItems: 'center',
            ...
            <OverlayTrigger placement="bottom" delay={{ show: 150, hide: 150 }} overlay={<Tooltip id="button-tooltip" style={{ fontSize: '12px' }}>
              ...
              <span>
                <button className="px-2 btn btn-link" onClick={handleDiagnosticsClick}>
                  <i className="fa fa-wrench" style={{ fontSize: 32, color: '#333', verticalAlign: 'middle' }}/>
                </button>
              </span>
            </OverlayTrigger>
            {isReadOnly && (<OverlayTrigger placement="bottom" delay={{ show: 150, hide: 150 }} overlay={<Tooltip id="button-tooltip" style={{ fontSize: '12px' }}>
                    Read Only
                  </Tooltip>}>
                <span>
                  <button className="px-2 btn btn-link" onClick={handleReadOnlyClick}>
                    <i className="fa fa-exclamation-triangle" style={{ fontSize: 40, color: 'goldenrod' }}/>
                  </button>
                </span>
              </OverlayTrigger>)}
          </div>
