...
import { selectBottomPanel, selectCurrentRule, selectHasEditingLock, selectLeftPanel, selectProjectSlug, selectReadOnly, selectRevisionSlug, selectRightPanel, selectShowDiagnosticsWindow, selectTopPanel, setInitialConfig, setPanelState, } from './store/app/slice';
...
const Authoring = (props) => {
    ...
    const shouldShowEditor = !isLoading && (hasEditingLock || isReadOnly) && !shouldShowReadOnlyWarning;
    ...
    const handlePanelStateChange = ({ top, right, left, bottom, }) => {
        dispatch(setPanelState({ top, right, left, bottom }));
    };
    const dismissReadOnlyWarning = ({ attemptEdit }) => __awaiter(void 0, void 0, void 0, function* () {
        var _a, _b;
        if (attemptEdit) {
            const attemptResult = yield dispatch(attemptDisableReadOnly());
            if (attemptResult.meta.requestStatus !== 'fulfilled') {
                const errorCode = (_b = (_a = attemptResult) === null || _a === void 0 ? void 0 : _a.payload) === null || _b === void 0 ? void 0 : _b.error;
    ...
    useEffect(() => {
        if (isAppVisible) {
            ...
            const darkModeCss = document.getElementById('authoring-theme-dark');
            darkModeCss.href = '/css/authoring_torus_light.css';
            document.body.classList.add('overflow-hidden'); // prevents double scroll bars
            authoringContainer === null || authoringContainer === void 0 ? void 0 : authoringContainer.classList.remove('d-none');
            setTimeout(() => {
                authoringContainer === null || authoringContainer === void 0 ? void 0 : authoringContainer.classList.add('startup');
            }, 50);
        }
        if (!isAppVisible) {
            ...
            authoringContainer === null || authoringContainer === void 0 ? void 0 : authoringContainer.classList.remove('startup');
            setTimeout(() => {
                authoringContainer === null || authoringContainer === void 0 ? void 0 : authoringContainer.classList.add('d-none');
            }, 350);
        }
        return () => {
            document.body.classList.remove('overflow-hidden');
    ...
    useEffect(() => {
        window.addEventListener('beforeunload', () => __awaiter(void 0, void 0, void 0, function* () {
            return isFirefox
                ? setTimeout(() => __awaiter(void 0, void 0, void 0, function* () {
                    yield dispatch(releaseEditingLock());
                }))
                : yield dispatch(releaseEditingLock());
        }));
        setTimeout(() => {
        ...
        return () => {
            window.removeEventListener('beforeunload', () => __awaiter(void 0, void 0, void 0, function* () { return yield dispatch(releaseEditingLock()); }));
        };
    }, [props, hasEditingLock, isReadOnly, isReadOnlyWarningDismissed]);
    return (<>
      {isLoading && (<div id="aa-loading">
          <div className="loader spinner-border text-primary" role="status">
            <span className="sr-only">Loading...</span>
          </div>
        </div>)}
      {shouldShowEditor && (<div id="advanced-authoring" className={`advanced-authoring d-none`}>
          <HeaderNav panelState={panelState} isVisible={panelState.top}/>
          <SidePanel position="left" panelState={panelState} onToggle={() => handlePanelStateChange({ left: !panelState.left })}>
            <LeftMenu />
          </SidePanel>
          <EditingCanvas />
          <BottomPanel panelState={panelState} onToggle={() => handlePanelStateChange({ bottom: !panelState.bottom })}>
            {currentRule === 'initState' && <InitStateEditor />}
            {currentRule !== 'initState' && <AdaptivityEditor />}
          </BottomPanel>
          <SidePanel position="right" panelState={panelState} onToggle={() => handlePanelStateChange({ right: !panelState.right })}>
            <RightMenu />
          </SidePanel>
        </div>)}

      {shouldShowReadOnlyWarning && (<Alert variant={alertSeverity}>
          <Alert.Heading>Opening in Read-Only Mode</Alert.Heading>
          {!isAttemptDisableReadOnlyFailed && (<p>
          ...
          <div style={{ textAlign: 'center' }}>
            <Button variant={`outline-${alertSeverity}`} className="text-dark" onClick={() => dismissReadOnlyWarning({ attemptEdit: false })}>
              Continue In Read-Only Mode
            </Button>{' '}
            {!isAttemptDisableReadOnlyFailed && (<>
                <Button variant={`outline-${alertSeverity}`} className="text-dark" onClick={() => dismissReadOnlyWarning({ attemptEdit: true })}>
                  Open In Edit Mode
                </Button>{' '}
              </>)}
            <Button variant={`outline-${alertSeverity}`} className="text-dark" onClick={() => window.open(url, windowName)}>
...
//# sourceMappingURL=Authoring.jsx.map