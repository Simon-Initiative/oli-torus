...
describe('short answer question', () => {
    ...
    it('can add and remove a response in text mode', () => {
        const updated = dispatch(model, ResponseActions.addResponse(makeResponse(containsRule('another answer'), 0, ''), DEFAULT_PART_ID));
        ...
        expect(dispatch(updated, ResponseActions.removeResponse(updated.authoring.parts[0].responses[1].id))
            .authoring.parts[0].responses).toHaveLength(2);
    });
    it('can add and remove a response in numeric mode', () => {
        ...
        updated = dispatch(updated, ResponseActions.addResponse(makeResponse(eqRule('1'), 0, ''), DEFAULT_PART_ID));
        expect(updated.authoring.parts[0].responses[0].score).toBe(1);
        expect(updated.authoring.parts[0].responses[1].score).toBe(0);
        expect(updated.authoring.parts[0].responses[2].score).toBe(0);
...
//# sourceMappingURL=short_answer_authoring_test.js.map