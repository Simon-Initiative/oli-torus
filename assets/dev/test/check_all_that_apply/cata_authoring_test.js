...
import { getChoiceIds, getCorrectResponse, getIncorrectResponse, getResponseId, getResponses, getTargetedResponses, } from 'data/activities/model/responses';
...
describe('check all that apply question functionality', () => {
    ...
    it('can toggle choice correctness', () => {
        ...
        const modelWithFirstChoiceToggled = dispatch(model, CATAActions.toggleChoiceCorrectness(firstChoice.id));
        expect(getChoiceIds(modelWithFirstChoiceToggled.authoring.correct)).not.toContain(firstChoice.id);
    });
    ...
    it('can remove a choice from targeted catas', () => {
        const firstChoice = model.choices[0];
        const newModel = dispatch(model, CATAActions.removeChoiceAndUpdateRules(firstChoice.id));
        newModel.authoring.targeted.forEach((assoc) => expect(getChoiceIds(assoc)).not.toContain(firstChoice.id));
    });
    ...
    it('can remove a targeted feedback', () => {
        const withNewResponse = dispatch(model, CATAActions.addTargetedFeedback());
        const removed = dispatch(withNewResponse, ResponseActions.removeTargetedFeedback(getResponseId(withNewResponse.authoring.targeted[0])));
...
//# sourceMappingURL=cata_authoring_test.js.map