...
describe('multiple choice question', () => {
    ...
    it('can add a choice', () => {
        expect(dispatch(model, Choices.addOne(makeChoice(''))).choices.length).toBeGreaterThan(model.choices.length);
    });
    it('can edit a choice', () => {
        const newChoiceContent = makeChoice('new content').content;
        const firstChoice = model.choices[0];
        expect(dispatch(model, Choices.setContent(firstChoice.id, newChoiceContent)).choices[0]).toHaveProperty('content', newChoiceContent);
    });
...
//# sourceMappingURL=mc_authoring_test.js.map