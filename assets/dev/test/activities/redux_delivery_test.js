...
import { initializeState } from 'data/activities/DeliveryState';
import { defaultActivityState, initialPartInputs, stringToStudentInput, studentInputToString, } from 'data/activities/utils';
...
describe('activity delivery state management', () => {
    ...
    it('can select single choices', () => {
        ...
        expect((_a = store.getState().partState[DEFAULT_PART_ID]) === null || _a === void 0 ? void 0 : _a.studentInput).toEqual([
            model.choices[0].id,
        ]);
    });
    ...
    it('can convert input to selections used by components and back', () => {
        ...
        expect(studentInputToString(selection)).toEqual(`${model.choices[0].id} ${model.choices[1].id}`);
        let input = '123';
...
//# sourceMappingURL=redux_delivery_test.js.map