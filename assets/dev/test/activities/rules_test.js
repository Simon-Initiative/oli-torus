import { andRules, btwRule, containsRule, matchListRule, eqRule, gteRule, gtRule, invertRule, lteRule, ltRule, matchRule, nbtwRule, neqRule, notContainsRule, orRules, matchInOrderRule, } from 'data/activities/model/rules';
describe('rules', () => {
    it('match rule', () => {
    ...
    it('between two numbers rule', () => {
        expect(btwRule('1', '2')).toBe('input = {2} || (input < {2}) && (input = {1} || (input > {1}))');
    });
    it('not between two numbers', () => {
        expect(nbtwRule('1', '2')).toBe('(!(input = {2} || (input < {2}) && (input = {1} || (input > {1}))))');
    });
    it('invert rule', () => {
        expect(invertRule(matchRule('id'))).toBe('(!(input like {id}))');
    });
    it('and rules', () => {
        expect(andRules(matchRule('id1'), invertRule(matchRule('id2')))).toBe('(!(input like {id2})) && (input like {id1})');
    });
    it('or rules', () => {
        expect(orRules(matchRule('id1'), matchRule('id2'))).toBe('input like {id2} || (input like {id1})');
    });
    ...
    it('can create rules to match certain ids and not match others', () => {
        ...
        expect(matchListRule(allChoiceIds, toMatch)).toEqual('(!(input like {id3})) && (input like {id2} && (input like {id1}))');
    });
});
//# sourceMappingURL=rules_test.js.map