...
3. Fill the doc for exactly one slice with AC mapping, signatures, data flow, and tests.
   ...
   - If scenario status is `Required` or `Suggested` for this slice's ACs/workflows, include concrete scenario artifacts and validation commands.
   - If status is `Not applicable`, state why this slice does not need scenario coverage.
   - Read PRD/FDD LiveView status and incorporate LiveView-test obligations in the slice test plan.
   - If LiveView status is `Required` or `Suggested` for this slice, include concrete LiveView test artifacts and commands.
   - If LiveView status is `Not applicable`, state why this slice does not need LiveView test coverage.
