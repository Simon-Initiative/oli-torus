...
- Goal: Provide ClickHouse health checks and a sample query library in admin UI.
- Tasks
  - [x] Implement `Oli.Analytics.ClickhouseAnalytics` (health check, sample queries, query execution, status/progress).
  - [ ] Remove admin-facing custom query execution; retain health checks and sample query library only.
  - [x] Add admin dashboard LiveView `/admin/clickhouse` with health check and sample query library.
  - [x] Gate admin access behind global `clickhouse-olap` feature flag.
- Definition of Done: Admins can validate ClickHouse health and review sample query definitions from the UI without executing arbitrary queries.
