...
- **Oli.GenAI.Router**: Decision module that produces a `RoutingPlan` from request context + ServiceConfig policy + live signals (counters + breaker state) and performs atomic admission for pool/model caps.
...
- **Admin UI Extension**: `OliWeb.GenAI.ServiceConfigsView` adds Secondary selection, routing parameters, and read-only health indicators; `OliWeb.GenAI.RegisteredModelsView` adds pool_class/max_concurrent fields and pool size controls.
...
3. Router reads ETS breaker snapshots and attempts atomic admission for pool + model caps to produce a `RoutingPlan` (selected model, tier, pool, reason, timeouts).
...
- `OliWeb.GenAI.ServiceConfigsView`:
  ...
  - Add read-only health display (breaker state, recent error/429/latency snapshot, active request counts).
...
- `Oli.GenAI.AdmissionControl`
  ...
  - `read/1`: return counters for routing and UI (per-service-config requests + per-model/pool inflight).
...
## 8. Caching Strategy
- ETS table `:genai_counters` for active request counts per ServiceConfig (UI) and inflight counts per model/pool (admission).
