...
`spec_ui_implement` is not part of the mandatory linear feature lane.

Use it as a supporting skill when UI work depends on Figma or another external design source.
It can support:

- `spec_analyze` in the full feature lane
- `spec_work` in the lightweight lane
- `spec_design` when a slice needs deeper UI-specific grounding

Its job is to produce a governed UI implementation brief covering tokens, icons, reusable components, and likely file targets before coding.

## Lightweight Workflow

Use this when the ticket does not justify full PRD/FDD/plan ceremony.

```mermaid
%%{init: {'flowchart': {'nodeSpacing': 5, 'rankSpacing': 20}}}%%
flowchart TD

JT[Jira Ticket / User Prompt]
JT --> SW(spec_work)
SW -->|if Figma link is present or provided| UII(spec_ui_implement)
UII --> UIBRIEF[/UI implementation brief/]
UIBRIEF -->|brief informs spec_work| SW
SW --> APPROVAL[Review / Approve]
APPROVAL --> IMPL[Implement]
IMPL --> REVIEW2[Review]
REVIEW2 --> QA2[QA]
QA2 --> OPEN2[Open PR]

classDef artifact fill:#fff7d1,stroke:#cfa74a,stroke-width:2px,color:#3b2b00;
class UIBRIEF artifact;
classDef skill fill:#dff5df,stroke:#2e7d32,stroke-width:2px,color:#1b5e20;
class SW,UII skill;
classDef manual fill:#f8e1ff,stroke:#8e44ad,stroke-width:2px,color:#4a235a;
class APPROVAL,IMPL,REVIEW2,QA2,OPEN2 manual;
```
...
- `spec_ui_implement` -- Converts Figma or another UI design source into a governed implementation brief with token, icon, component, and file-target mapping.
