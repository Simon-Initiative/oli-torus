...
- Assumptions:
  ...
  - Assessment titles and course-context labels can be resolved outside the UI from `GradesOracle` rows enriched with section-resource metadata, with `ScopeResourcesOracle` remaining the fallback source for scope-aware labels.
...
- What we know:
  ...
  - The merged `MER-5252-student-support-tile-P3` implementation now establishes the most relevant email-flow reference: `EmailButton` remains a thin trigger, while the dashboard tile owns a richer draft-email modal specialized for tile-managed recipients rather than the older generic students-table modal.
