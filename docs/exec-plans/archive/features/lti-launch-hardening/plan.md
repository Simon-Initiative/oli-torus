...
- When section setup launches `/users/link_account`, the link-account flow must preserve a validated local return path so successful linking returns to section setup instead of a generic workspace.
