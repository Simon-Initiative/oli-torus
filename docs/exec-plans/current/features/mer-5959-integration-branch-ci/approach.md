...
- [ ] Smoke test on GitHub with a disposable `integ-ci-smoke` branch, created from a
      temporary commit of the working tree so the ticket branch itself stays
      uncommitted. Smoke PRs are titled
      `[DEVOPS] [MER-5959] CI smoke test - do not merge`, and the child PR's change
      is a throwaway file that never reaches `master` or the ticket branch:
  - pushing `integ-ci-smoke` runs build and Playwright, and the naming validation
    passes;
  - a draft PR `integ-ci-smoke` → `master` runs build, Playwright, title
    validation, and preview, and skips Danger and AI review;
  - a PR into `integ-ci-smoke` runs build, Playwright, title validation, Danger,
    AI review, and preview, and `https://preview-<N>.plasma.oli.cmu.edu/`
    answers 401 (deployed behind auth) rather than 404;
  - squash merging it triggers the push runs on `integ-ci-smoke` and re-runs the
    draft PR checks;
  - creating a malformed branch (for example `integ_ci_smoke`) and opening a PR
    against it both fail the naming validation;
  - close the smoke PRs without merging and delete every disposable branch
    afterwards.
