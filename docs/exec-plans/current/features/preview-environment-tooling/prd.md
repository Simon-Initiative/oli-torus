...
- Preserve scenario seeding in `dev`, `test`, and `ci_e2e`, expose privileged release seeding in `preview`, and explicitly exclude every supported seeding entry point from `prod`.
...
- Security: scenario seeding is limited to `dev`, `test`, `ci_e2e`, and `preview`; `prod` exposes no seeding entry point. `MIX_ENV=preview` controls inclusion in deployable preview releases, `MIX_ENV=test` may compile seeding sources for verification, release-shell invocation activates and authorizes `bin/seed`, and `PREVIEW_QA_TOOLS_ENABLED` independently controls web-accessible masquerade and mailbox features. Playwright interfaces require their automation token; masquerade independently requires a current system administrator at every boundary.
