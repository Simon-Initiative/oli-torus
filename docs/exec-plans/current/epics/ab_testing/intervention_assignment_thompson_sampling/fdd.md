...
- Assignment: owns the sticky selection under the experiment's assignment scope and supplies the stable visibility decision used by rendering and learner-specific progress/completion denominator construction. Thompson Sampling and intervention-scoped weighted random use one enrollment/intervention identity; section-and-enrollment weighted random uses one experiment/section/enrollment identity. Publication and page-revision snapshots are not assignment state; they are captured on assignment/exposure evidence events.
...
- `experiment_assignments`: require `experiment_id` and an explicit assignment scope. Intervention-scoped rows require `intervention_id` and are unique by `(experiment_id, intervention_id, enrollment_id)`; section-and-enrollment weighted-random rows omit intervention ownership and are unique by `(experiment_id, section_id, enrollment_id)`. Retain section/user and compact policy evidence; no `decision_point_id` is stored.
