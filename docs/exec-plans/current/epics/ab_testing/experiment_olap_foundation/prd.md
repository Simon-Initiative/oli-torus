...
- ClickHouse-backed query contracts must support scoped reads by institution, project, section, experiment, decision point, condition, learner/enrollment reference where allowed, publication where available, algorithm, policy version, attribution role, underlying attribution type, host event type, and event timestamp from the attribution projection. `role` describes the attribution's relationship to its host statement, while `attribution_type` preserves the semantic evidence type (`assignment`, `exposure`, `outcome`, `reward`, or `policy_update`) through rollup and media transformations. Detailed page/resource/activity/media context is read by joining `experiment_attributions.raw_event_hash` back to `raw_events.event_hash`.
