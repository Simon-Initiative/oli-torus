...
- Show the current policy and guardrail status from the same snapshot: normal Thompson Sampling, warm-up weighted random, fixed-control enforcement, or traffic-cap enforcement with affected conditions. Show configured thresholds and current progress where applicable, surface assignment-imbalance monitoring as a warning rather than a selection constraint, and use the experiment lifecycle state—not `manual_pause_enabled` configuration—to indicate whether assignment is paused or ended.
...
Existing experiment definitions, assignments, rewards, analytics, Alternatives Groups, placements, and page content require no feature-specific conversion or backfill. Existing content elements may retain a legacy `strategy` field; the field is ignored rather than migrated, removed, synchronized, or used as a fallback. Existing Alternatives Group revisions and publications with `upgrade_decision_point` remain unchanged and are normalized at read boundaries to the canonical `experiment_controlled` behavior. New group revisions and newly ingested groups write only `experiment_controlled`; editing a legacy group may upgrade its new revision through the normal authoring and publication flow, without forcing republication of historical or deployed revisions. Legacy imported groups without an explicit group strategy retain the existing `user_section_preference` default. New PostgreSQL and ClickHouse schema changes must follow the normal deployment path and preserve existing content compatibility. Active behavior begins only for valid newly configured decision points; completed and archived history remains immutable.
...
- Automated validation:
  ...
  - JSON Schema tests proving `priv/schemas/v0-1-0/content-alternatives.schema.json` accepts the new group-reference-only placement shape and supported legacy placements while rejecting placements without a valid `alternatives_id`.
