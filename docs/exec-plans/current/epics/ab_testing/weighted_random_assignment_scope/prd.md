...
- Make section-and-enrollment scope the default for new weighted-random experiments while preserving Thompson Sampling's intervention-scoped assignment model and existing experiments unchanged.
...
- Section-and-enrollment scope must be the default for new weighted-random experiments.
...
Existing experiments retain intervention scope and their current assignments unchanged. New weighted-random experiments default to section-and-enrollment scope and may explicitly select intervention scope subject to lifecycle rules. Thompson Sampling remains intervention-scoped. No backfill or consolidation of existing intervention-scoped assignments is performed. Rollback must preserve dependency-safe schema behavior but is not required to transform experiment-level assignments into equivalent intervention-level history.
