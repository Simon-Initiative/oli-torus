...
Add a typed `assignment_scope` to experiment definitions with two values: `intervention` and `section_enrollment`. Existing and new experiments default to `intervention`. Only weighted-random experiments may use `section_enrollment`; Thompson Sampling remains intervention-scoped.
...
- Functional requirements:
  - FR-001 and AC-001 add author-configurable weighted-random assignment scope with intervention as the default.
...
1. A new experiment form defaults `assignment_scope` to `intervention`.
...
- `CreateExperimentRequest` adds `assignment_scope: :intervention | :section_enrollment`, defaulting to `:intervention`.
...
- Public request callers that omit `assignment_scope` continue to create intervention-scoped experiments through the request default.
...
- The earlier `intervention_assignment_thompson_sampling` documentation names course-wide sticky assignment as a non-goal and independent intervention assignment as universal behavior. A documentation-reconciliation task must update that wording to describe intervention scope as the default and this weighted-random-only exception without changing Thompson Sampling requirements.
