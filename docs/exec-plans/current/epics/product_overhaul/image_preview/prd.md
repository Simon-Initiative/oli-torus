...
## 1. Overview
MER-4052 (`image_preview`) adds cover-image mockups to Template Overview so authors and admins can see a selected cover image as students will see it in My Course, Course Picker, and the Student Welcome page. The feature must reuse the same runtime rendering units that production student surfaces use so preview fidelity is preserved automatically when those surfaces change, while matching the gallery and modal behavior shown in the linked Figma designs.
...
- Keep preview output in lockstep with production by extracting and reusing canonical rendering templates or components from the destination student surfaces.
...
- Template maintainer: updates shared destination UI markup and expects preview output to reflect the same change without a second implementation pass.
...
- Student: indirectly benefits because template managers validate the exact runtime presentation rather than an approximation.
...
- Accessibility: the preview experience must preserve accessible names, semantic structure, keyboard reachability, and visible focus behavior expected of the shared runtime surfaces.
...
- Shared rendering logic should stay in the Phoenix view or component layer so both runtime surfaces and preview surfaces consume the same implementation without duplicating markup.
...
Telemetry is not required for this work item. Success is established by matching the MER-4052 acceptance criteria and Figma interaction states for no-image, uploaded-gallery, hover, and modal-carousel behavior while preserving runtime rendering parity across the three student contexts.
...
- Automated validation:
  - LiveView or view-layer coverage proving each preview context reuses the same rendering unit as its corresponding destination surface.
