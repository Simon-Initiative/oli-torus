...
9. Node `44-805` — reference card for source type "Template" (a product/Blueprint). **Discrepancy found (2026-09-18):** this card's Figma pill actually reads "Free" (green), not "Template" as the PRD/Jira AC calls for. Interim decision (Gastón): implement the literal text "Template" per the AC, reusing the same green pill styling; a Slack question is pending to Jess (design) to confirm whether "Free"/cost-based is actually intended instead, and if so what a paid template should show. If a source is neither "My Section" nor "Template", it appears to carry no label at all.
10. Node `44-1459` — "My Section" card hover state: same card with a darkened overlay and the label "SELECT TO COPY COURSE SECTION". This hover sits on a card that is genuinely selectable (see the 2026-09-18 scope correction above) — the label accurately describes what clicking already does. **Open question from Gastón:** whether other card types (Free/Template) get an equivalent hover treatment — needs designer confirmation.
...
- Restyle the source cards per type: "My Section" and "Template" get distinct treatments; a source that is neither gets no label at all (design references 8-9). The "Template" tag's exact text/color is an interim choice pending design confirmation — see Open decisions.
...
- **Pending Slack question to Jess (design), asked 2026-09-18:** the "Template" tag's Figma reference card (node `44-805`) shows a green "Free" pill instead of the word "Template" the PRD/Jira AC calls for. Interim decision until she answers: implement literal "Template" text, reusing the same green pill styling. If she confirms cost-based ("Free") was intended instead, this needs a follow-up change, and a still-open question about what a *paid* template's card should show.
