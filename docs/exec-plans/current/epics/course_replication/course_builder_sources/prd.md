...
- Redesigned source cards: denser layout, a "Template" identification tag for existing project/product sources, a "My Section" identification tag for My Course Sections rows, and no identification tag for a source that is neither. Separately, the existing cost badge ("Free" or a price, currently `TableModel.render_payment_column/3` rendered as a plain badge) is restyled to a pill treatment and scoped to display on product/Template cards only; it is a distinct label from the identification tag and both can appear on the same card at once (e.g. a free Template shows "Template" and "Free" together).
...
### Open Questions
- Whether the restyled cost badge should be scoped to display on product/Template cards only, as inferred from the Figma reference cards (present on the Template card, absent on the My Section and untagged cards), versus continuing to render unconditionally on every card as today's code does — Jess confirmed the badge is separate from the identification tag and that both can coexist, but did not explicitly confirm the display-scoping inference; proceeding with product/Template-only scoping as the reading most consistent with the mockups.
- No Figma reference exists for a paid Template's card (what replaces "Free" isn't shown) — not inventing it; reusing `render_payment_column/3`'s existing price-string behavior as-is.
- Whether the My Section hover treatment (dark overlay + "SELECT TO COPY COURSE SECTION") should extend to Template cards, or stays exclusive to My Section cards — no other card type shows this hover state in the designs reviewed so far; needs designer confirmation.
...
### Assumptions
- Per Jess (design, Slack, 2026-09-18): the "Free" pill visible on the Template reference card is the existing, unrelated cost badge, restyled — not the new "Template" identification tag, which remains a separate label per the PRD/Jira AC and can coexist with the cost badge on the same card.
