...
Detailed subsystem design for data/oracle/caching internals is maintained in `docs/epics/intelligent_dashboard/data.md`.
...
- `Tile Dependency`: The required/optional oracle set declared by a tile. A tile can fully render when required dependencies are ready and may partially render with optional dependencies pending.
...
- `LiveDataCoordinator`: The runtime controller that handles async load orchestration, in-flight/queued request policy, and stale-result suppression during fast scope changes.
...
- Keep dashboard infrastructure reusable across products (i.e. other, future dashboards like Authoring)
...
- Specific Oracle implementations for instructor use cases
- Tile composition and dependency declarations
...
### 4) Tile Consumption Model

Tiles do not query storage directly.

- Tiles declare required and optional oracle dependencies.
- Tiles render incrementally as required oracle data becomes available.
...
- Resolve required oracles for visible tiles.
- Do not perform initial `InProcess` cache lookup (guaranteed miss on true initial mount).
...
sequenceDiagram
    ...
    LDC->>OR: start_oracle_loads(default_container, token_0)
...
- On parameterized return navigation (like to a specific scope), consult short-lived per-user revisit cache.
- If found, rapidly hydrate prior container oracles.
- Otherwise continue normal in-process/build path.
...
sequenceDiagram
    ...
    LDC->>RC: lookup(user_id, dashboard_context, container_x, required_oracles)
    RC-->>LDC: hit / miss

    alt Revisit cache hit
        LDC->>IPC: write returned oracle payloads
        LDC-->>LV: hydrate from revisit payloads
        LV-->>U: Fast container render
    else Revisit cache miss
        LDC->>OR: start_oracle_loads(container_x, token_r)
        OR->>O: instantiate container-specific oracle set
        loop each oracle completion
            O-->>OR: oracle_n payload
...
- `Revisit Cache`: short-lived node-wide per-user cache for one recently viewed container.
- Capacity controls are container-count based (not oracle-count based).
- Freshness is TTL-driven; detailed policy is defined in `data.md`.
...
- New tiles should declare oracle dependencies and consume projections.
...
- `data.md`: subsystem-level design for data/oracles/caching.
