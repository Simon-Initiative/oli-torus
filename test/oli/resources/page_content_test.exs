defmodule Oli.Resources.PageContentTest do
  ...
  @content %{
    "model" => [
      %{"type" => "group", "layout" => "deck", "children" => [
        %{"type" => "group", "layout" => "vertical", "children" => [
  ...
  describe "map support via map_reduce" do
    test "it numbers all the nodes" do
      {item, _} = PageContent.map_reduce(@content, 0, fn item, acc -> {Map.put(item, "id", acc + 1), acc + 1} end)
  ...
  describe "flat filter support" do
    test "it flattens and filters all activity references" do
      refs = PageContent.flat_filter(@content, fn item -> Map.get(item, "type") == "activity-reference" end)
