defmodule Oli.GenAI.Dialogue.FallbackTest do
  ...
  test "falls back to NullProvider when primary provider fails" do
    ...
    primary = %RegisteredModel{provider: :open_ai, model: "foo", url_template: "https://api.thisserverdoesnotexist.com"}
    backup  = %RegisteredModel{provider: :null, model: "bar", url_template: "https://api.null.com/v1/chat/completions"}
    ...
    :timer.sleep(1000) # Allow time for the fallback to process
    ...
    assert Enum.at(state.messages, 2).content == "This is a null provider. No generation performed. "
  end
end
