defmodule Oli.InstructorDashboard.DataSnapshot.Projections.SummaryTest do
  ...
  describe "required_oracles/0 and optional_oracles/0" do
    test "declares shared summary metrics as optional oracle inputs" do
  ...
  describe "derive/2" do
    test "builds concrete summary scope and metric fields when optional inputs are present" do
      assert {:ok, projection} = Summary.derive(snapshot_fixture(), [])
    ...
    test "returns a partial projection with nil metric fields when optional inputs are missing" do
      snapshot =
        snapshot_fixture(%{
          oracle_instructor_grades: nil,
          oracle_instructor_objectives_proficiency: nil
        })

      assert {:partial, projection, {:dependency_unavailable, missing}} =
               Summary.derive(snapshot, [])
      ...
      assert is_nil(projection.metrics.average_class_proficiency)
      assert projection.total_students == 12
    end
  end
  ...
  defp snapshot_fixture(overrides \\ %{}) do
    default_oracles = %{
      ...
      oracle_instructor_progress_proficiency: [
        %{student_id: 1, progress_pct: 40.0},
        %{student_id: 2, progress_pct: 80.0}
      ],
      oracle_instructor_grades: %{
        grades: [
      ...
      oracle_instructor_objectives_proficiency: %{
        objective_rows: [
          %{objective_id: 1001, proficiency_distribution: %{"Low" => 2, "Medium" => 1}},
          %{objective_id: 1002, proficiency_distribution: %{"High" => 2, "Medium" => 1}}
        ]
      }
    }
