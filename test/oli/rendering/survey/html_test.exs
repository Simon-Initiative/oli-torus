defmodule Oli.Content.Survey.HtmlTest do
  ...
  describe "html activity renderer" do
    ...
    test "renders well-formed survey properly", %{author: author} do
      ...
      assert rendered_html_string =~
               ~s|<p>Please complete the following survey:</p>\n<oli-multiple-choice-delivery phx-update=\"ignore\" id=\"activity_1\" class="activity-container" graded="false" state="{ "active": true }" model="{ "choices": [ "A", "B", "C", "D" ], "feedback": [ "A", "B", "C", "D" ], "stem": ""}" mode="delivery"|
