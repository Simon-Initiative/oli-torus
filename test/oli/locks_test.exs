defmodule Oli.LocksTest do
  ...
  describe "locks" do
    ...
    test "acquire/3 does acquire the lock", %{author: author, publication: publication, container: %{ resource: container_resource } } do
    ...
    test "can acquire then release", %{author: author, publication: publication, container: %{resource: resource}} do
    ...
    test "can only release if acquired", %{author: author, author2: author2, publication: publication, container: %{ resource: resource }} do
      assert Locks.acquire(publication.id, resource.id, author.id) == {:acquired}
      assert Locks.release(publication.id, resource.id, author2.id) == {:lock_not_held}
    end

    test "updating a lock", %{author: author, author2: author2, publication: publication, container: %{resource: resource}} do
    ...
    test "acquiring an expired lock", %{author: author, author2: author2, publication: publication, container: %{ resource: resource }} do
