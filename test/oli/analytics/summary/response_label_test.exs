defmodule Oli.Analytics.Summary.ResponseLabelTest do
  ...
  test "choice labelling" do
    ...
    assert %ResponseLabel{response: "2", label: "B"} = attempt_with_response("2")
    |> ResponseLabel.build("oli_multiple_choice")

    assert %ResponseLabel{response: "", label: "No answer"} = attempt_with_response(nil)
    |> ResponseLabel.build("oli_multiple_choice")

    assert %ResponseLabel{response: "2 4 6", label: "B, D, F"} = attempt_with_response("2 4 6")
    |> ResponseLabel.build("oli_check_all_that_apply")

    assert %ResponseLabel{response: "", label: "No answer"} = attempt_with_response(nil)
    |> ResponseLabel.build("oli_check_all_that_apply")

    assert %ResponseLabel{response: "1 2 3 4 5 6", label: "A, B, C, D, E, F"} = attempt_with_response("1 2 3 4 5 6")
    |> ResponseLabel.build("oli_ordering")

    assert %ResponseLabel{response: "", label: "No answer"} = attempt_with_response(nil)
    |> ResponseLabel.build("oli_ordering")

    assert %ResponseLabel{response: "3", label: "C"} = attempt_with_response("3")
    |> ResponseLabel.build("oli_image_hotspot")

    assert %ResponseLabel{response: "2", label: "2"} = attempt_with_response("2")
    |> ResponseLabel.build("oli_likert")

    assert %ResponseLabel{response: "2", label: "5"} = attempt_with_response("2")
    |> set_descending()
    |> ResponseLabel.build("oli_likert")
  ...
  test "text response labelling" do
    ...
    assert %ResponseLabel{response: "here is my answer", label: "here is my answer"} = attempt_with_response("here is my answer")
  ...
  test "multi input" do
    ...
    assert %ResponseLabel{response: "25.4", label: "25.4"} = attempt_with_response("25.4", "part2")
    |> ResponseLabel.build("oli_multi_input")

    assert %ResponseLabel{response: "3", label: "A"} =  attempt_with_response("3", "part3")
    |> ResponseLabel.build("oli_multi_input")
  ...
  test "custom dnd" do
    ...
    assert %ResponseLabel{response: "part1", label: "A"} =  attempt_with_response("part1")
  ...
  test "unsupported" do
    ...
    assert %ResponseLabel{response: "unsupported", label: "unsupported"} = attempt_with_response("something")
