defmodule Oli.PublishingTest do
  ...
  describe "publishers" do
    test "create_publisher/1 with valid data creates a publisher" do
      ...
      assert {:ok, %Publisher{} = publisher} = Publishing.create_publisher(params)
    ...
    test "create_publisher/1 with existing name returns error changeset" do
      ...
      assert {:error, %Ecto.Changeset{}} = Publishing.create_publisher(%{name: publisher.name})
    end
    ...
    test "list_publishers/0 returns all the publishers" do
      ...
      assert 3 = length(Publishing.list_publishers())
    end

    test "get_publisher/1 returns a publisher when the id exists" do
      publisher = insert(:publisher)

      returned_publisher = Publishing.get_publisher(publisher.id)
    ...
    test "get_publisher/1 returns nil if the publisher does not exist" do
      assert nil == Publishing.get_publisher(123)
    end

    test "update_publisher/2 updates the publisher successfully" do
      publisher = insert(:publisher)

      {:ok, updated_publisher} = Publishing.update_publisher(publisher, %{name: "new_name"})
    ...
    test "update_publisher/2 does not update the publisher when there is an invalid field" do
      ...
      {:error, changeset} =
        Publishing.update_publisher(publisher, %{name: another_publisher.name})
    ...
    test "delete_publisher/1 deletes the publisher" do
      publisher = insert(:publisher)
      assert {:ok, deleted_publisher} = Publishing.delete_publisher(publisher)
      refute Publishing.get_publisher(deleted_publisher.id)
    end

    test "change_publisher/1 returns a publisher changeset" do
      publisher = insert(:publisher)
      assert %Ecto.Changeset{} = Publishing.change_publisher(publisher)
    end
  end
end
