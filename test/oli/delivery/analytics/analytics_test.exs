defmodule Oli.Delivery.Analytics.AnalyticsTest do
  ...
  def seed_snapshots(context) do
    ...
    {:ok, %{
      ...
      objective_query: ByObjective.query_against_project_id(seeds.project_id),
      page_query: ByPage.query_against_project_id(seeds.project_id)
    }}
  end
  ...
  describe "number of attempts" do
    ...
    test "with no attempts", %{ activity_query: activity_query, page_query: page_query, seeds: seeds } do
      ...
      # same for pages, objecti
      page_no_attempts = seeds.page1.revision
      page_results = Enum.find(page_query, & &1.slice.id == page_no_attempts.id)
      assert page_results.number_of_attempts == nil
    end

    test "activity with attempts from 1 user", %{ activity_query: activity_query, seeds: seeds } do
    ...
    test "activity with attempts from multiple users", %{ activity_query: activity_query, seeds: seeds } do
    ...
    test "multiple activities", %{ activity_query: activity_query, seeds: _seeds } do
  ...
  describe "relative difficulty" do
    ...
    test "no incorrect answers and no hints", %{ activity_query: activity_query, seeds: seeds } do
      ...
      activity_results = Enum.find(activity_query, & &1.slice.id == activity_no_incorrect_no_hints.id)
    ...
    test "one user - hints and incorrect answers", %{ activity_query: activity_query, seeds: seeds } do
      ...
      activity_results = Enum.find(activity_query, & &1.slice.id == activity_one_user_hints_and_incorrect.id)

      assert activity_results.relative_difficulty == 2
    end

    test "multiple users - hints and incorrect answers", %{ activity_query: activity_query, seeds: seeds } do
      # this activity has attempts from two users:
      ...
      activity_results = Enum.find(activity_query, & &1.slice.id == activity_mult_users_hints_and_incorrect.id)
  ...
  describe "eventually correct" do
    ...
    test "all users get correct answer eventually", %{ activity_query: activity_query, seeds: seeds } do
      ...
      activity_results = Enum.find(activity_query, & &1.slice.id == activity_mult_users_correct.id)
      assert activity_results.eventually_correct == 1
    end

    test "not all users get correct answer eventually", %{ activity_query: activity_query, seeds: seeds } do
      # this activity has 6 attempts from 2 users (3 each)
      ...
      activity_results = Enum.find(activity_query, & &1.slice.id == activity_mult_users_correct_incorrect.id)
