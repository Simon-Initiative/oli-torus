defmodule Oli.Delivery.Gating.Strategies.ScheduleTest do
  use Oli.DataCase

  describe "schedule strategy" do
    alias Oli.Delivery.Gating
    alias Oli.Delivery.Gating.Strategies.Schedule

    setup do
      Seeder.base_project_with_resource4()
    end

    test "check/1 returns true when current time is inside of the scheduled window", %{page1: resource, section_1: section} do
      today = DateTime.utc_now()
      yesterday = today |> DateTime.add(-(24 * 60), :second)
      tomorrow = today |> DateTime.add((24 * 60), :second)

      {:ok, gating_condition} = Gating.create_gating_condition(%{
        type: :schedule,
        data: %{
    ...
    test "check/1 returns false when current time is before the scheduled window", %{page1: resource, section_1: section} do
      today = DateTime.utc_now()
      tomorrow = today |> DateTime.add((24 * 60), :second)
    ...
    test "check/1 returns false when current time is after the scheduled window", %{page1: resource, section_1: section} do
      today = DateTime.utc_now()
      yesterday = today |> DateTime.add(-(24 * 60), :second)
      two_days_before_now = today |> DateTime.add(-(2 * 24 * 60), :second)
    ...
    test "check/1 handles nil values for start and/or end datetimes", %{page1: resource, section_1: section} do
      today = DateTime.utc_now()
      yesterday = today |> DateTime.add(-(24 * 60), :second)
      tomorrow = today |> DateTime.add((24 * 60), :second)
