defmodule Oli.InstitutionsTest do
  ...
  describe "pending_registration" do
    ...
    setup do
      ...
      %{institution: institution, jwk: jwk, registration: registration, deployment: deployment, pending_registration: pending_registration}
    end
    ...
    test "list_pending_registrations/0 returns all pending_registrations", %{pending_registration: pending_registration} do
    ...
    test "get_pending_registration!/1 returns the pending_registration with given id", %{pending_registration: pending_registration} do
      assert Institutions.get_pending_registration!(pending_registration.id) == pending_registration
    end

    test "create_pending_registration/1 with valid data creates a pending_registration" do
      assert {:ok, %PendingRegistration{} = pending_registration} = Institutions.create_pending_registration(@valid_attrs)
    ...
    test "create_pending_registration/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Institutions.create_pending_registration(@invalid_attrs)
    end

    test "update_pending_registration/2 with valid data updates the pending_registration", %{pending_registration: pending_registration} do
      assert {:ok, %PendingRegistration{} = pending_registration} = Institutions.update_pending_registration(pending_registration, @update_attrs)
      assert pending_registration.name == "some updated institution"
    end

    test "update_pending_registration/2 with invalid data returns error changeset", %{pending_registration: pending_registration} do
      assert {:error, %Ecto.Changeset{}} = Institutions.update_pending_registration(pending_registration, @invalid_attrs)
      assert pending_registration == Institutions.get_pending_registration!(pending_registration.id)
    end

    test "delete_pending_registration/1 deletes the pending_registration", %{pending_registration: pending_registration} do
      assert {:ok, %PendingRegistration{}} = Institutions.delete_pending_registration(pending_registration)
      assert_raise Ecto.NoResultsError, fn -> Institutions.get_pending_registration!(pending_registration.id) end
    end

    test "change_pending_registration/1 returns a pending_registration changeset", %{pending_registration: pending_registration} do
