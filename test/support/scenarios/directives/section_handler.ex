defmodule Oli.Scenarios.Directives.SectionHandler do
  ...
  def handle(%SectionDirective{name: name, title: title, from: from, type: type, registration_open: reg_open}, state) do
    try do
      section = if from do
  ...
  defp create_from_project(project_name, title, type, reg_open, state) do
    case Engine.get_project(state, project_name) do
      ...
      built_project ->
        ...
        publication = case Publishing.get_latest_published_publication_by_slug(built_project.project.slug) do
          nil ->
  ...
  defp create_standalone(title, type, reg_open, state) do
    ...
    {:ok, project} = Oli.Authoring.Course.create_project(%{
      title: "#{title} Project",
    ...
    # Publish it
    {:ok, publication} = Publishing.publish_project(
      project,
      "initial",
      state.current_author.id
    )
