defmodule Oli.Scenarios.Directives.Assert.ProficiencyAssertion do
  ...
  def assert(%AssertDirective{proficiency: proficiency}, state) when is_map(proficiency) do
    with {:ok, section} <- get_section(state, proficiency.section),
         ...
         {:ok, actual_proficiency} <- calculate_proficiency(
           section,
           objective,
           state,
           proficiency
      ...
      value_matches = if expected_value do
      ...
      message = if passed do
        ...
        parts = []
        parts = if !bucket_matches do
          ["Expected bucket '#{expected_bucket}' but got '#{actual_bucket}'"] ++ parts
        else
          parts
        end
        parts = if expected_value && !value_matches do
  ...
  defp find_objective_by_title(state, objective_title) do
    ...
    objective = state.projects
      |> Enum.flat_map(fn {_name, project} ->
        # Get objectives from the project
        case Map.get(project, :objectives) do
          nil -> []
          objectives -> find_objective_in_list(objectives, objective_title)
        end
      end)
      |> List.first()
  ...
  defp find_objective_in_list(objectives, title) do
    objectives
      |> Enum.flat_map(fn obj ->
        # Check if this objective matches
        matches = if Map.get(obj, :title) == title do
          [obj]
        else
          []
        end
        
        # Also check sub-objectives
        sub_matches = case Map.get(obj, :children) do
          nil -> []
          [] -> []
          children -> find_objective_in_list(children, title)
        end
  ...
  defp calculate_student_proficiency(section, objective, state, proficiency) do
    ...
    if user == nil do
      ...
      proficiency_map = Metrics.proficiency_for_student_per_learning_objective(
        learning_objectives,
        user.id,
        section
      )
  ...
  defp calculate_average_proficiency(section, objective, _state, _proficiency) do
    ...
    student_ids = Sections.list_enrollments(section.slug)
      |> Enum.filter(& &1.user_role_id == 4)  # Student role
      |> Enum.map(& &1.user_id)
    ...
    if Enum.empty?(student_ids) do
      ...
      raw_data = from(summary in Oli.Analytics.Summary.ResourceSummary,
        where: summary.section_id == ^section.id and
      ...
      if Enum.empty?(raw_data) do
        ...
        {total_value, total_count} = raw_data
          |> Enum.reduce({0.0, 0}, fn {first_correct, first_count, _correct, _total}, {sum, count} ->
            if first_count > 0 do
        ...
        if total_count > 0 do
          ...
          bucket = Metrics.proficiency_range(avg_value, 3)  # Use 3 as minimum for "enough data"
          {:ok, {avg_value, bucket}}
        else
          {:ok, {0.0, "Not enough data"}}
        end
