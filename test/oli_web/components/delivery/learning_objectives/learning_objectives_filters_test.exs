defmodule OliWeb.Components.Delivery.LearningObjectives.FiltersTest do
  ...
  describe "filtered_objectives/2 and objectives_count/1" do
    ...
    test "filters by the Not Enough Data proficiency option (id 4)" do
      objectives = [
        %{
          resource_id: 1,
          title: "LO.02",
          objective: "LO.02",
          subobjective: nil,
          student_proficiency_obj: "Not enough data",
          student_proficiency_subobj: nil,
          container_ids: [10]
        },
        %{
    ...
    # Regression test: Confidence has no "Not Enough Data" option (unlike Proficiency,
    # which has id 4), and its missing-data state is nil rather than a label string.
    # Option id 4 must be ignored for Confidence, not silently match nothing.
    test "ignores option id 4 for Confidence instead of matching nothing" do
      objectives = [
        %{
          resource_id: 1,
          title: "LO.02",
      ...
      assert Enum.any?(filtered, &(&1.objective == "LO.02"))
      assert Enum.any?(filtered, &(&1.objective == "LO.99"))
    end
  end
end
