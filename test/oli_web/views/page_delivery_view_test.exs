    ...
    test "renders shrinkable text containers so long titles can truncate" do
      prev_html =
        render_component(&PageDeliveryView.prev_link/1, %{
          to: "/previous",
          title: "A very long page title that should truncate instead of pushing the arrow off screen"
        })

      next_html =
        render_component(&PageDeliveryView.next_link/1, %{
          to: "/next",
          title: "Another very long page title that should truncate instead of pushing the arrow off screen"
        })
