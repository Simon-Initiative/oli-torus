defmodule OliWeb.Delivery.StudentDashboard.StudentDashboardLiveTest do
  ...
  describe "learning_objectives" do
    ...
    test "container navigator dropdown is suppression-aware and keeps a suppressed middle unit in its document position",
      ...
      # Before the fix, this navigator read raw SectionResourceDepot.containers/2 output
      # with no suppression overlay at all -- "Beta" (suppressed) would show a raw "Unit 2"
      # label instead of no number, and "Gamma" would show its raw "Unit 3" instead of the
      # renumbered "Unit 2". It must match Learn: Beta unnumbered, Gamma renumbered, and
      # every item in its real document position.
      ...
      assert normalized_text =~ "Unit 1: Alpha"
      refute normalized_text =~ "Unit 2: Beta"
      refute normalized_text =~ "Unit 3: Gamma"
      assert normalized_text =~ "Unit 2: Gamma"
    end
  end
