defmodule OliWeb.ObjectivesControllerTest do
  ...
  describe "POST operation to create an objective" do
    ...
    test "creates an objective as a child objective", %{conn: conn, project: project, child1: child1} do

      conn = post(conn, Routes.objectives_path(conn, :create, project.slug), %{"title" => "new one", "parentId" => child1.resource.id})
    ...
    test "creates a top-level objective", %{conn: conn, project: project} do
      ...
      conn = post(conn, Routes.objectives_path(conn, :create, project.slug), %{"title" => "new one"})
    ...
    test "fails when project does not exist", %{conn: conn} do
      ...
      conn = post(conn, Routes.objectives_path(conn, :create, "this_id_does_not_exist"), %{"title" => "new title"})
      assert response(conn, 404)
  ...
  describe "PUT operation to update an objective" do
    ...
    test "updates the title of an objective", %{conn: conn, project: project, child1: child1} do
      ...
      conn = put(conn, Routes.objectives_path(conn, :update, project.slug, child1.resource.id), %{"title" => "new title"})
    ...
    test "fails when objective does not exist", %{conn: conn, project: project} do
      ...
      conn = put(conn, Routes.objectives_path(conn, :update, project.slug, 919191919), %{"title" => "new title"})
      assert response(conn, 404)

    end

    test "fails when title is missing from payload", %{conn: conn, project: project, child1: child1} do

      conn = put(conn, Routes.objectives_path(conn, :update, project.slug, child1.resource.id), %{})
      assert response(conn, 400)
    ...
    test "fails when project does not exist", %{conn: conn, child1: child1} do
      ...
      conn = put(conn, Routes.objectives_path(conn, :update, "this_id_does_not_exist", child1.resource.id), %{"title" => "new title"})
      assert response(conn, 404)
