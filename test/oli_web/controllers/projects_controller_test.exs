defmodule OliWeb.ProjectsControllerTest do
  ...
  describe "export_csv/2" do
    setup do
      admin = Oli.AccountsFixtures.author_fixture(%{system_role_id: SystemRole.role_id().system_admin})
    ...
    test "exports respect sorting parameters", %{conn: conn, admin: admin} do
      ...
      # Test ascending sort
      conn = get(conn, ~p"/authoring/projects/export?sort_by=title&sort_order=asc&text_search=Admin")
      csv_content = response(conn, 200)
      ...
      titles = Enum.map(valid_lines, fn line ->
    ...
    test "CSV properly escapes special characters", %{conn: conn, admin: admin} do
      ...
      {:ok, %{project: _project}} = Course.create_project("Project, with \"quotes\" and\nnewlines", admin)
