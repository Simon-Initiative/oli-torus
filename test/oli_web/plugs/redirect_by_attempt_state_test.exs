defmodule OliWeb.Plugs.RedirectByAttemptStateTest do
  ...
  describe "call/2 for graded pages" do
    ...
    test "redirects graded adaptive chromeless to adaptive review when review path", %{
      ...
      assert_redirected_to_path(
        result_conn,
        ~p"/sections/#{section.slug}/adaptive_lesson/#{page_revision.slug}/attempt/test-attempt/review"
      )
    end

    # Test: {:graded, :not_adaptive, _, true} -> ensure_path(conn, :review, :not_adaptive)
    test "redirects graded not adaptive to review when review path", %{
      conn: conn,
      section: section,
      ...
      assert_redirected_to_path(
        result_conn,
        ~p"/sections/#{section.slug}/lesson/#{page_2_revision.slug}/attempt/test-attempt/review"
      )
    end

    # Test: {:graded, _, nil, false} -> ensure_path(conn, :prologue)
    test "redirects graded adaptive chromeless to prologue when no attempt", %{
      conn: conn,
      section: section,
      page_revision: page_revision
    } do
    ...
    test "redirects graded to prologue when submitted attempt", %{
      conn: conn,
      section: section,
      page_revision: page_revision,
      user: user
    ...
    test "redirects graded not adaptive to lesson when active attempt", %{
      conn: conn,
      section: section,
      page_2_revision: page_2_revision,
      user: user
  ...
  describe "call/2 for practice pages" do
    ...
    test "redirects practice not adaptive to lesson when not review path", %{
      conn: conn,
      section: section,
      page_2_revision: page_2_revision
    } do
    ...
    test "redirects practice adaptive chromeless to adaptive lesson when not review path", %{
      conn: conn,
      section: section,
      page_revision: page_revision
    } do
    ...
    test "redirects practice adaptive chromeless to adaptive review when review path", %{
      conn: conn,
      section: section,
      page_2_revision: page_2_revision
    } do
    ...
    test "does not redirect when already been redirected", %{
      conn: conn,
      section: section,
      page_2_revision: page_2_revision
    } do
