...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(QZ)) -> {ok, QZ} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(QT)) -> {ok, QT} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(QP)) -> {ok, QP} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(RF), fun((RF) -> json())) -> json().
...
-spec array(list(RJ), fun((RJ) -> json())) -> json().
...
-spec dict(gleam@dict:dict(RN, RO), fun((RN) -> binary()), fun((RO) -> json())) -> json().
